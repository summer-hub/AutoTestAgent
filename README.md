...
- **仓库同步**：`pull_repo` / `update_repo` 真实 git 拉取更新 + 变更文件解析，支持弹窗输入仓库地址直接拉取，内置「仓库目录」浏览服务器本地代码；`to_script` 脚本落盘
...
#    "dsh-autotest": "https://github.com/summer-hub/AutoTestAgent/releases/download/v0.1.6/dsh-autotest-0.1.6.tgz"
#    然后必须执行安装（光写不装等于没写）：
...
也可以把 tgz 下载到本地后用 `"dsh-autotest": "file:./dsh-autotest-0.1.6.tgz"` 或 `pnpm add ./dsh-autotest-0.1.6.tgz`，离线环境更稳；第 2~5 步不变。
