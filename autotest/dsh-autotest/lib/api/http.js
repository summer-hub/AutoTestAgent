...
import { analyzeAttribution, analyzeCaseUpdates, analyzePrChanges, fetchPrs, parseRepoPath, } from '../services/analyzer.js';
...
function defineRoutes(llm) {
    ...
    route('PUT', '/cases/:id', async ({ params, body }) => {
        ...
        const updated = db.transaction(() => {
            ...
            db.prepare(`UPDATE cases SET name=?, source=?, precondition=?, steps=?, expected=?, status=?, script_status=?, current_version=?, updated_at=? WHERE id=?`).run(snapshot.name, snapshot.source, snapshot.precondition, JSON.stringify(snapshot.steps), snapshot.expected, snapshot.status, snapshot.scriptStatus, nextVersion, t, id);
            db.prepare(`INSERT INTO case_versions (case_id, version, snapshot, change_note, author, author_type, created_at)
    ...
    route('POST', '/devices/scan', async () => {
      ...
      VALUES (?, ?, ?, 'online', ?, ?, ?, ?)`).run(serial, model, os, 60 + Math.floor(Math.random() * 40), 40 + Math.floor(Math.random() * 40), t, t);
        ...
        return { discovered: res.changes > 0, device: mapDevice(row), total: db.prepare('SELECT COUNT(*) AS n FROM devices').get().n };
    });
    ...
    route('POST', '/analyses/pr/:libraryId', async ({ params }) => {
        ...
        return analyzePrChanges(llm, lib, prs);
    });
    route('POST', '/analyses/case-updates/:libraryId', async ({ params }) => {
        ...
        return analyzeCaseUpdates(llm, lib, prs);
    });
