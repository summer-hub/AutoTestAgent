...
import { getAllSettings, getSetting, maskSettingValue, SECRET_SETTING_KEYS, setSetting } from '../services/settings.js';
 ...
 */
function invalidateCaseCaches(opts = {}) {
    void cacheDel('cases');
    void cacheDel('stats');
    if (opts.libCounts !== false) {
        void cacheDel('lib');
        void cacheDel('libs');
    }
}
    ...
    return new Promise((resolve, reject) => {
        ...
        req.on('data', (c) => {
            ...
            if (overflow > 0 || size + c.length > MAX_BODY_BYTES) {
                ...
                if (overflow > MAX_BODY_BYTES * 8) {
                    req.destroy();
                    settle(() => reject(Object.assign(new Error(`请求体过大（超过 ${Math.round(MAX_BODY_BYTES / 1024 / 1024)}MB）`), { statusCode: 413 })));
                }
                return;
            }
            size += c.length;
        ...
        req.on('end', () => settle(() => {
            ...
            try {
                ...
                reject(Object.assign(new Error('请求体不是合法 JSON（已拒绝，避免按空对象继续写库）'), { statusCode: 400 }));
            }
        }));
...
function assertSameOrigin(req) {
    if (String(process.env.AUTOTEST_ALLOW_CROSS_ORIGIN || '') === '1')
        return;
 ...
 */
function assertSafeModelBaseUrl(raw) {
    const s = String(raw ?? '').trim();
    if (!s)
        throw Object.assign(new Error('baseUrl 必填'), { statusCode: 400 });
    let u;
    try {
        u = new URL(s);
    }
    catch {
        throw Object.assign(new Error(`baseUrl 不是合法 URL：${s}`), { statusCode: 400 });
    }
    if (u.protocol !== 'http:' && u.protocol !== 'https:') {
        throw Object.assign(new Error(`baseUrl 仅支持 http/https，收到 ${u.protocol}`), { statusCode: 400 });
    }
    const host = u.hostname.toLowerCase();
    const blocked = /^169\.254\./.test(host) ||
...
function defineRoutes(llm) {
    ...
    route('POST', '/workspace/open', async ({ body }) => {
        ...
        child.on('error', (e) => console.warn(`[dsh-autotest] 调起 ${cmd} 失败（目录已创建）：${e.message}`));
        child.unref();
    ...
    route('PUT', '/settings/:key', async ({ params, body }) => {
        ...
        if (SECRET_SETTING_KEYS[key] && isMaskedSecret(value)) {
            return { ok: true, key, value: maskSettingValue(key, getSetting(key, '')), unchanged: true };
        }
    ...
    route('GET', '/libraries', async ({ query }) => {
        ...
        return withCache(`libs:${page}:${pageSize}:${q}:${cursor}`, () => withRead(async (db) => {
            ...
            const withCases = (await db.prepare(`SELECT COUNT(*) AS n FROM libraries l ${where ? `${where} AND` : 'WHERE'} EXISTS (SELECT 1 FROM cases c WHERE c.library_id = l.id)`).get({ like, cursor }))?.n ?? 0;
    ...
    route('POST', '/libraries/:id/coverage-matrix/export', async ({ params, body }) => {
        ...
        if (!lib)
            throw Object.assign(new Error('库不存在'), { statusCode: 404 });
        const format = String(body?.format ?? 'md').toLowerCase() === 'csv' ? 'csv' : 'md';
        const r = await loadCoverageMatrix(id);
        if (r.rows.length === 0)
            throw Object.assign(new Error('该库还没有覆盖矩阵，请先构建。'), { statusCode: 400 });
    ...
    route('POST', '/libraries/:id/demo-scenarios/export', async ({ params, body }) => {
        ...
        if (r.rows === 0)
            throw Object.assign(new Error('该库还没有场景清单/覆盖率报告，请先跑 demo 场景与覆盖分析两个阶段。'), { statusCode: 400 });
        const dir = path.join(workspaceDir(), 'coverage', lib.name.replace(/[^\w.-]/g, '_'));
        fs.mkdirSync(dir, { recursive: true });
    ...
    route('POST', '/scripts/run', async ({ body }) => {
        ...
        const name = String(b.name ?? '').trim();
        if (!libId || !/^[\w.-]+\.py$/i.test(name))
            throw Object.assign(new Error('libraryId / name(.py) 必填'), { statusCode: 400 });
        const db = getDb();
        const lib = await db.prepare('SELECT name, package_name FROM libraries WHERE id = ?').get(libId);
        if (!lib)
            throw Object.assign(new Error('三方库不存在'), { statusCode: 404 });
        if (!(await hdcAvailable()))
            throw Object.assign(new Error('真机未连接：未检测到 hdc 命令。请连接鸿蒙设备后重试'), { statusCode: 400 });
        const targets = await listTargets();
        if (targets.length === 0)
        ...
        VALUES (?, 1, ?, 'Excel 导入初始创建', 'Excel 导入', 'human', ?)`);
            for (let ri = 0; ri < raw.length; ri++) {
                ...
                try {
                    ...
                    errors.push(`第 ${ri + 2} 行：${e.message}`);
                }
            }
        });
        invalidateCaseCaches();
    ...
    route('PUT', '/models/:id', async ({ params, body }) => {
        ...
        await db.prepare(`UPDATE models SET name=?, provider=?, base_url=?, model_id=?, api_key=?, is_default=?, updated_at=? WHERE id=?`).run(b.name ?? row.name, b.provider ?? row.provider, baseUrl, b.modelId ?? row.model_id, apiKey, b.isDefault === undefined ? row.is_default : (b.isDefault ? 1 : 0), t, id);
        if (b.isDefault)
    ...
    // ---- prompts ----
    route('GET', '/prompts', async () => withCache('prompts', async () => (await getDb().prepare('SELECT * FROM prompts ORDER BY builtin DESC, id').all()).map(mapPrompt)));
    ...
    route('POST', '/tasks', async ({ body }) => {
      ...
      VALUES (?, ?, ?, ?, ?, '[]', ?, 'pending', 0, ?, ?)`).run(taskNo, b.type, title, b.libraryId ?? null, b.input ?? '', traceId, t, t);
    ...
    route('GET', '/tasks/:id/events', async ({ params, query }) => {
        const id = Number(params.id);
        const row = await getDb().prepare('SELECT id FROM tasks WHERE id = ?').get(id);
        if (!row)
            throw Object.assign(new Error('任务不存在'), { statusCode: 404 });
    ...
    route('POST', '/tasks/:id/retry', async ({ params }) => {
        const id = Number(params.id);
        const db = getDb();
        const row = await db.prepare('SELECT status, library_id FROM tasks WHERE id = ?').get(id);
        if (!row)
            throw Object.assign(new Error('任务不存在'), { statusCode: 404 });
        if (row.status !== 'failed' && row.status !== 'cancelled')
            throw Object.assign(new Error('仅失败/已取消任务可重试'), { statusCode: 400 });
    ...
    route('POST', '/tasks/:id/cancel', async ({ params }) => {
        ...
        if (r === 'already_finished')
            throw Object.assign(new Error('任务已终态，无需取消'), { statusCode: 400 });
    ...
    route('POST', '/plans', async ({ body }) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'draft', ?, ?, '', ?, ?)`).run(planNo, b.name, b.type, b.cron ?? null, JSON.stringify(scope), JSON.stringify(deviceIds), b.failPolicy ?? 'continue', scriptMode, t, t);
        const id = Number(res.lastInsertRowid);
        ...
        if (b.type === 'immediate') {
            ...
            setImmediate(() => { executePlan(id).catch((e) => console.error(`[plan #${id}] 执行失败：`, e.message)); });
        }
...
用户的追问：${question}`;
    ...
    // ---- devices ----
    route('GET', '/devices', async () => withCache('devices', async () => (await getDb().prepare(`SELECT * FROM devices ORDER BY status = 'online' DESC, id`).all()).map(mapDevice)));
    route('POST', '/devices/scan', async () => {
        ...
        if (scan.ok && scan.detected > 0) {
            const row = await db.prepare(`SELECT * FROM devices WHERE status = 'online' ORDER BY last_seen_at DESC, id LIMIT 1`).get();
            return {
                discovered: true,
        ...
        if (!scan.ok) {
            // hdc 不可用：明确报错，引导安装
            throw Object.assign(new Error('未检测到 hdc 命令或服务不可用。请安装 HarmonyOS Device Connector（hdc）并配置环境变量后，通过 USB 连接鸿蒙机型设备'), { statusCode: 400 });
        }
        // hdc 可用但没有设备
        return {
            discovered: false,
    ...
    route('GET', '/analyses/export', async ({ query }) => {
        ...
        if (round) {
            conds.push('round = @round');
            p.round = round;
        }
        const where = conds.length ? `WHERE ${conds.join(' AND ')}` : '';
        const rows = await getDb().prepare(`SELECT * FROM analyses ${where} ORDER BY id DESC LIMIT 5000`).all(p);
        const data = rows.map((r) => {
            const c = safeJsonParse(r.content, {});
            ...
            return {
                ...
                更新点: Array.isArray(c.updatePoints) ? c.updatePoints.join('\n') : String(c.updatePoints ?? ''),
                影响范围: String(c.impact ?? ''),
                受影响功能: Array.isArray(c.affectedFeatures) ? c.affectedFeatures.join('\n') : '',
                风险: String(c.risk ?? ''),
                建议用例更新: Array.isArray(c.suggestedCaseUpdates) ? c.suggestedCaseUpdates.join('\n') : '',
                轮次: String(r.round ?? ''),
                分析时间: String(r.created_at ?? ''),
            };
        });
    ...
    route('GET', '/explore/reports/:libraryId/summary', async ({ params }) => {
        ...
        let report;
        try {
            report = JSON.parse(fs.readFileSync(path.join(dir, latest), 'utf8'));
        }
        catch {
            return { ok: true, report: null, pages: [], cases: [], stats: null };
        }
        // 关联用例：读取该库全部 case_versions snapshot 的 pagePath（最近一次遍历生成的，来源 AI 生成）
        const versionRows = await db.prepare(`SELECT v.case_id, v.snapshot, c.case_no, c.name, c.script_status, c.status
       FROM case_versions v JOIN cases c ON c.id = v.case_id
       WHERE c.library_id = ? AND c.source = 'AI 生成'`).all(lib.id);
