...
export function apply(ctx) {
    ...
    ensureAuthSchema()
        ...
        try {
            ...
            const [rows] = await db.query('SELECT COUNT(*) AS n FROM auth_users');
            if (rows[0].n === 0) {
            ...
            console.error('[dsh-autotest] 初始管理员创建失败：', e.message);
        }
    })
        .catch((e) => {
        console.error('[dsh-autotest] 认证库初始化失败：', e.message);
    });
