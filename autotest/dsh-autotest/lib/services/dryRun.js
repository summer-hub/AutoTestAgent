 ...
 */
export async function dryRunCase(caseNo, caseName, steps, serial, opts = {}) {
    const perStep = opts.perStepTimeoutMs ?? 15000;
    const stopAfter = Math.max(1, opts.failStreakStop ?? 2);
    const logs = [`[dry-run] 设备 ${serial} 开始执行 ${caseNo}（${steps.length} 步）`];
    const out = [];
    let passed = true;
    let streak = 0;
    if (opts.launch) {
