...
export function hypiumLibSlug(libName) {
...
export function hypiumCaseSlug(caseNo) {
 ...
 */
export function hypiumCaseModule(libName, caseNo) {
 ...
 */
export function hypiumProjectDir(_libName) {
    return path.join(workspaceDir(), 'hypium');
}
/** 用例的库目录：<工程根>/testcases/<lib>。 */
export function hypiumLibDir(libName) {
 ...
 */
export function migrateLegacyHypiumLayouts() {
    ...
    for (const entry of fs.readdirSync(root, { withFileTypes: true })) {
        if (!entry.isDirectory())
            continue;
        if (entry.name === 'testcases' || entry.name === 'aw' || entry.name === 'config' || entry.name === 'resource')
            continue;
        const legacyRoot = path.join(root, entry.name);
        // 判定为旧工程根：有 main.py 且其下还有 testcases/
        if (!fs.existsSync(path.join(legacyRoot, 'main.py')))
            continue;
        ...
        for (const f of fs.readdirSync(legacyRoot)) {
            if (skeleton.has(f)) {
                try {
                    fs.rmSync(path.join(legacyRoot, f), { force: true });
                }
                catch { /* 忽略 */ }
            }
        }
...
function removeEmptyDirs(dir) {
    ...
    for (const f of fs.readdirSync(dir, { withFileTypes: true })) {
        if (f.isDirectory())
            removeEmptyDirs(path.join(dir, f.name));
    }
    try {
 ...
 */
export function oracleToPython(o, caseNoHint = '', packageName = '') {
    const q = (v) => py(String(v ?? ''));
    switch (String(o.type ?? '')) {
        case 'script_assert': {
            const expr = String(o.expr ?? '').trim();
            if (!expr)
                return [`        raise AssertionError("oracle 缺少断言表达式")`];
 ...
 */
export function oracleSupportModuleSource() {
    return [
        '# coding: utf-8',
        '"""',
 ...
 */
export function caseClassName(libName, caseNo) {
 ...
 */
export function caseJsonContent(lib, c) {
...
export function generateCaseScript(lib, c) {
    ...
    for (let i = 0; i < c.steps.length; i++) {
        ...
        if (lines === null)
            throw new UnmappedStepError(i, c.steps[i]);
    ...
    if (needsSupport) {
        const fns = new Set();
        if (c.steps.some((s) => /^\s*(验证|断言|检查|校验)/.test(s)))
            fns.add('assert_control_text');
        for (const o of oracles) {
 ...
 */
export function scriptHasAssertion(script) {
...
export function writeCaseScript(lib, c) {
    ...
    if (!scriptHasAssertion(script))
        throw new NoAssertionError(c.caseNo);
