...
export async function listBindings(libraryId) {
    const db = getDb();
    ...
    WHERE c.library_id = ? ORDER BY b.status, c.id`).all(libraryId);
...
async function judge(r) {
    ...
    return {
        ...
        lastRunStatus: String(r.last_run_status ?? ''), lastRunAt: r.last_run_at ?? null,
        confirmedAt, updatedAt: String(r.updated_at ?? ''), fileExists,
    };
}
/** 全库重算状态并落库（前端列表直接读 status 列时可省一次计算；判定逻辑仍以 judge 为准）。 */
export async function refreshBindingStatuses(libraryId) {
    const db = getDb();
 ...
 */
export async function bindCaseScript(caseId, opts = {}) {
    const db = getDb();
    ...
    // 人工改过的脚本默认不覆盖（设计明确要求）
    const existing = await db.prepare('SELECT * FROM case_script_bindings WHERE case_id = ?').get(caseId);
    if (existing) {
    ...
    try {
        ...
        if (e instanceof UnmappedStepError)
            return { ok: false, reason: e.message, unmappedStep: { index: e.stepIndex, step: e.step } };
        if (e instanceof NoAssertionError)
...
export async function unbindCaseScript(caseId, opts = {}) {
    const db = getDb();
    const row = await db.prepare('SELECT script_path FROM case_script_bindings WHERE case_id = ?').get(caseId);
    if (!row)
        throw Object.assign(new Error('该用例没有脚本绑定'), { statusCode: 404 });
    let removedFile = '';
...
export async function recordScriptRun(caseId, status) {
    const db = getDb();
    const t = now();
 ...
 */
export async function markBindingStaleOnCaseBump(caseId, newVersion) {
    const db = getDb();
...
export function scriptDirFor(libName) {
