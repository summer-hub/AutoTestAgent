...
export function symbolCases(symbolId, caseRows, links) {
    ...
    for (const c of caseRows) {
        if (c.api_symbol_id !== symbolId || seen.has(c.case_no))
            continue;
    ...
    return {
        ...
        evidence: {
            ...
            caseRefs: cases.map((c) => ({
                ...
                basis: c.basis ?? '', confidence: c.confidence ?? 'high',
                caseName: c.name,
            })),
...
export function summarizeMatrix(rows, extra = {}) {
...
export async function buildCoverageMatrix(libraryId) {
    ...
    for (const list of linksBySymbol.values())
        for (const l of list)
            linkedCaseIds.add(l.caseId);
