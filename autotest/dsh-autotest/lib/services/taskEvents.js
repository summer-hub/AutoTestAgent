...
function chain(taskId, fn) {
    const prev = chains.get(taskId) ?? Promise.resolve();
    // 前一个失败也要继续跑：一条轨迹写失败不该让后续轨迹全部断掉
    const next = prev.then(fn, fn);
    const tracked = next.then(() => { if (chains.get(taskId) === tracked)
        chains.delete(taskId); }, () => { if (chains.get(taskId) === tracked)
        chains.delete(taskId); });
    chains.set(taskId, tracked);
    return next;
}
async function insertEvent(taskId, type, title, detail) {
    const at = now();
    await getDb().prepare(`INSERT INTO task_events (task_id, seq, type, title, detail, created_at)
     SELECT ?, COALESCE(MAX(seq), 0) + 1, ?, ?, ?, ? FROM task_events WHERE task_id = ?`).run(taskId, type, title, detail, at, taskId);
    const row = await getDb().prepare('SELECT seq FROM task_events WHERE task_id = ? ORDER BY seq DESC LIMIT 1').get(taskId);
    return row?.seq ?? 0;
}
/**
 ...
 * 否则 trace 视图看不到取消现场。
 */
export async function appendTrace(taskId, title, detail = '') {
...
export async function readTaskEvents(taskId, opts = {}) {
    ...
    const rows = await getDb().prepare(`SELECT seq, type, title, detail, created_at FROM task_events
     WHERE task_id = ? AND seq > ? ORDER BY seq ASC LIMIT ?`).all(taskId, afterSeq, limit);
...
export async function rebuildTraceSnapshot(taskId) {
    const rows = await getDb().prepare('SELECT seq, title, detail, created_at FROM task_events WHERE task_id = ? ORDER BY seq ASC').all(taskId);
