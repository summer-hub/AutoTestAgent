...
async function runHypiumModule(pythonCmd, projDir, moduleStem, timeoutMs) {
    ...
    if (fs.existsSync(reportsDir)) {
        ...
        if (fresh.length > 0)
            latest = fresh.sort().pop() ?? '';
        else
            latest = fs.readdirSync(reportsDir).sort().pop() ?? '';
    }
...
export async function executePlan(planId) {
    ...
    let deviceSerial = '';
    const wantedIds = JSON.parse(plan.device_ids || '[]');
    for (const id of wantedIds) {
        const d = await db.prepare('SELECT serial FROM devices WHERE id = ?').get(id);
        if (d && targets.includes(d.serial)) {
    ...
    for (const c of sample) {
        ...
        if (!groups.has(libName))
            groups.set(libName, []);
        groups.get(libName).push(item);
    }
    const total = [...groups.values()].flat().length;
    const deviceIdRow = await db.prepare('SELECT id FROM devices WHERE serial = ?').get(deviceSerial);
    ...
    for (const [libName, items] of groups) {
        ...
        if (boundItems.length === 0)
            continue;
        // 工程骨架 + 设备序列号刷新
        const pkgRow = await db.prepare('SELECT package_name FROM libraries WHERE name = ?').get(libName);
        ...
        for (const item of boundItems) {
            ...
            await insExec.run(planId, item.row.id, item.row.library_id, deviceId, status, JSON.stringify([
                ...
                : `在真实设备（${deviceSerial}）上执行绑定脚本 ${item.moduleStem}.py 失败。\n${result.log}`, [`[${now()}] 设备 ${deviceSerial} · ${item.row.case_no} · python main.py ${item.moduleStem}`, result.log].join('\n'), t, now());
            done++;
            if (status === 'passed')
                passed++;
            else
