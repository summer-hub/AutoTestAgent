...
export async function executePlan(planId) {
    ...
    // （设备争抢 + 执行记录翻倍）。
    const claim = await db.prepare(`UPDATE plans SET status='running', error='', progress=2, progress_note='准备中…', updated_at=? WHERE id=? AND status <> 'running'`).run(t, planId);
...
async function runPlanOnce(planId, plan, t) {
    const db = getDb();
    ...
    for (const c of sample) {
        ...
        const item = { row, scriptPath: bound ? scriptPath : null, moduleStem: path.basename(scriptPath, '.py'), projDir: hypiumProjectDir(libName) };
