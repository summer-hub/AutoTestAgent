...
export function parseEntry(md) {
    ...
    for (const rawLine of fmText.split(/\r?\n/)) {
        ...
        if (inEvidence && item) {
            try {
                evidence.push(JSON.parse(item[1]));
            }
            catch {
                evidence.push({ note: item[1] });
            }
            continue;
        }
        inEvidence = false;
    ...
    const scopeKind = (meta.scope_kind ?? 'global');
    const kind = (meta.kind ?? 'human_verdict');
    const status = (meta.status ?? 'ai_draft');
...
const SCOPE_PRIORITY = { api: 5, control: 4, library: 3, scenario: 2, global: 1 };
 ...
 */
export function retrieve(query, entries, budgetChars = 1500) {
    ...
    const must = [];
    for (const kind of ['library', 'scenario']) {
    ...
    for (const s of ordered) {
        ...
        if (used + size > budgetChars && selected.length >= must.length) {
            truncated++;
            continue;
        }
...
export function renderInjection(selected) {
...
export function entryDir(e) {
    const root = wikiRoot();
    const safe = (s) => s.replace(/[^\w.-]/g, '_').slice(0, 60) || '_';
    switch (e.scopeKind) {
...
export async function saveEntry(e) {
    ...
    const entry = { ...e, updatedAt: e.updatedAt ?? new Date().toISOString(), wikiPath: path.relative(wikiRoot(), file).replace(/\\/g, '/') };
...
export async function rebuildIndex() {
    ...
    await db.transaction(async () => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
                .run(e.wikiPath, e.scopeKind, e.scopeKey, e.kind, e.title, e.keywords.join(' '), e.status, e.confidence, JSON.stringify(e.evidence), contentHash(renderEntry(e)), e.createdAt, e.updatedAt);
        }
    });
 ...
 */
export function canConfirm(e) {
    if (e.status === 'human_confirmed')
        return { ok: true, reason: '已是人工确认状态' };
...
export async function deprecateStaleApiEntries(libraryId) {
    const db = getDb();
    const lib = await db.prepare('SELECT name FROM libraries WHERE id = ?').get(libraryId);
    if (!lib)
        return { checked: 0, deprecated: [] };
 ...
 */
export function kindForStage(stage) {
    switch (stage) {
        ...
        case 'video':
        case 'device_blocked':
        case 'untestable': return 'blocked_reason';
...
export function injectForStage(stage, query) {
    const budgets = { explore: 800, case_draft: 1500, case_optimize: 1500, triage: 800, script_gen: 1200 };
