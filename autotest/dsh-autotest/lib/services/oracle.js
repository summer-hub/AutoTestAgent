 ...
 */
export function validateOracles(oracles) {
    const problems = [];
    ...
    oracles.forEach((raw, index) => {
        ...
        if (!o || typeof o !== 'object') {
            problems.push({ index, reason: 'oracle 不是对象' });
            return;
        }
        ...
        switch (o.type) {
            case 'control_text':
                if (!o.control?.trim())
                    problems.push({ index, reason: 'control_text 缺少目标控件文本' });
                if (o.control && VAGUE_VALUES.test(o.control.trim()))
                    problems.push({ index, reason: `control_text 的目标控件「${o.control}」过于笼统，无法定位` });
                if (o.expect !== 'disappear')
                    o.expect = 'appear';
                break;
            case 'text_value':
                if (!o.control?.trim())
                    problems.push({ index, reason: 'text_value 缺少目标控件文本' });
                if (!o.value?.trim())
                    problems.push({ index, reason: 'text_value 缺少期望值' });
                ...
                if (!['equals', 'contains', 'matches'].includes(String(o.op)))
                    o.op = 'contains';
                break;
            case 'hilog_keyword':
                if (!o.keyword?.trim() && !o.value?.trim())
                    problems.push({ index, reason: 'hilog_keyword 缺少关键字' });
                break;
            case 'state_flag':
                if (!o.control?.trim())
                    problems.push({ index, reason: 'state_flag 缺少目标控件文本' });
                if (typeof o.state !== 'boolean')
                    problems.push({ index, reason: 'state_flag 缺少期望状态（true/false）' });
                break;
            case 'no_crash':
                if (!o.level)
                    o.level = 'E';
            ...
            case 'screenshot_diff':
                if (typeof o.threshold !== 'number' || o.threshold < 0 || o.threshold > 1)
                    problems.push({ index, reason: 'screenshot_diff 缺少合法的阈值（0-1）' });
                break;
            case 'script_assert':
                if (!o.expr?.trim())
                    problems.push({ index, reason: 'script_assert 缺少断言表达式' });
                else if (/^assert\s+(true|1)\b/i.test(o.expr.trim()))
                    problems.push({ index, reason: 'script_assert 是恒真断言（assert true），等于没有断言' });
                break;
            default:
                break;
        }
...
/** 用例是否达到"可机器校验"的入库门槛。 */
export function hasVerifiableOracle(oracles) {
 ...
 */
export function detectFalsePass(oracles, exec) {
    if (!exec.passed)
        return { falsePass: false, severity: 'none', reason: '用例未通过：不属于假通过（失败本身是有效信号）' };
...
export async function evaluateQuality(libraryId) {
    ...
    for (const r of rows) {
        ...
        if (problems.length === 0)
            ...
            missingOracleCases.push({ caseNo: r.case_no, name: r.name, reason: problems.map((p) => p.reason).join('；').slice(0, 200) });
    }
    ...
    for (const r of passed) {
        ...
        if (v.falsePass)
            falsePassCases.push({ caseNo: r.case_no, name: r.name, reason: v.reason });
    }
...
export async function saveCaseOracles(caseId, oracles) {
