...
function hasMachineSubstitute(stage, oracles) {
...
export function classifyAutomation(input, opts = {}) {
    const maxDuration = opts.maxDurationSec ?? 300;
    const flakyWindow = opts.flakyWindow ?? 3;
    const blockers = [];
    const reasons = [];
    const oracles = Array.isArray(input.oracles) ? input.oracles : [];
    ...
    for (const sig of SIGNALS) {
        if (!sig.re.test(text))
            continue;
        if (hasMachineSubstitute(sig.stage, oracles))
            continue;
        if (!blockers.includes(sig.stage))
            blockers.push(sig.stage);
...
export function buildQuestion(blockers, input) {
    const ask = [];
    ...
    if (blockers.includes('external_dep'))
        ask.push('准备外部依赖（数据库/服务/第二台设备）并手动执行，或在用例里改为不依赖外部条件的等价场景');
    if (blockers.includes('animation'))
        ask.push('人工观察动画是否符合预期，并填写结论（如可截图对比，请改为 screenshot_diff 判据后重新分流）');
    if (blockers.includes('video'))
        ask.push('人工观察音视频播放是否符合预期，并填写结论');
    if (blockers.includes('flaky'))
        ask.push('确认是否为设备/环境抖动；若是环境问题请标注并重跑，若用例本身不稳定请修正步骤');
    if (blockers.includes('device_blocked'))
        ask.push('连接一台在线真机或启动模拟器后重新分流');
    if (blockers.includes('long_running'))
        ask.push('确认是否可接受该时长；可接受请在系统配置里调高时长阈值，否则拆分用例');
    if (blockers.includes('untestable'))
        ask.push('确认该用例是否必要；若必要请给出人工验证方案与结论');
 ...
 */
export async function runTriage(libraryId, opts = {}) {
    const db = getDb();
    ...
    FROM cases WHERE library_id = ? ORDER BY id`)
        .all(libraryId);
    const device = await db.prepare("SELECT COUNT(*) AS n FROM devices WHERE status = 'online'").get();
    ...
    for (const h of history) {
        ...
        if (arr.length < 5 && (h.status === '通过' || h.status === '失败'))
            arr.push(h.status);
    ...
    for (const c of cases) {
        let oracles = [];
        try {
            oracles = JSON.parse(c.oracle_json || '[]');
        }
        catch {
            oracles = [];
        }
        ...
        const verdict = classifyAutomation({
            ...
            hasPatchDraft: !!c.demo_patch_json && (() => { try {
                return JSON.parse(c.demo_patch_json).edits?.length ? true : false;
            }
            catch {
                return false;
            } })(),
...
export async function loadHumanQueue(libraryId, status) {
    const db = getDb();
    ...
    WHERE q.library_id = ?${status ? ' AND q.status = ?' : ''} ORDER BY q.status, q.id`)
        .all(...(status ? [libraryId, status] : [libraryId]));
    return rows.map((r) => ({
        ...
        resolvedBy: String(r.resolved_by ?? ''), resolvedAt: r.resolved_at ?? null,
 ...
 */
export async function resolveQueueItem(itemId, resolution, resolvedBy = 'human') {
    const db = getDb();
    const row = await db.prepare('SELECT * FROM human_queue WHERE id = ?').get(itemId);
    if (!row)
        throw Object.assign(new Error('队列条目不存在'), { statusCode: 404 });
    const t = now();
    ...
    let knowledgeId = '';
    try {
        const lib = await db.prepare('SELECT name FROM libraries WHERE id = ?').get(Number(row.library_id));
        const caseRow = row.case_id
            ? await db.prepare('SELECT case_no FROM cases WHERE id = ?').get(Number(row.case_id))
            : undefined;
        ...
        console.warn('[autotest] 知识沉淀失败：', e.message);
    }
