...
export async function purgeSimulatedDevices() {
    try {
        ...
        const r = await getDb().prepare(`DELETE FROM devices WHERE serial LIKE 'HDC-%' AND length(serial) <= 8 AND model IN (${marks})`).run(...SIMULATED_MODELS);
 ...
 */
export async function autoScanDevices() {
    if (scanning)
        return { ok: false, detected: -1, reason: '扫描进行中' };
    scanning = true;
    try {
        const db = getDb();
        const t = now();
        if (!(await hdcAvailable()))
            return { ok: false, detected: 0, reason: 'hdc 不可用' };
        ...
        console.warn('[autotest] 设备自动检测失败：', e.message);
...
export function startDeviceAutoScan() {
    ...
    const timer = setInterval(() => {
        ...
        if (intervalSec <= 0)
            return; // 0 = 关闭自动检测
        if (Date.now() - lastScanAt < intervalSec * 1000)
            return;
