...
export function unregisterScheduledPlan(planId) {
...
export function stopAllSchedulers() {
    ...
    for (const job of backgroundJobs.splice(0))
        job.stop();
...
export async function startScheduler() {
    ...
    for (const p of plans) {
        try {
            ...
            const job = cron.schedule(p.cron, () => { executePlan(p.id).catch((e) => console.error(`[autotest] 计划 #${p.id} 执行失败：`, e.message)); });
            jobs.set(p.id, job);
    ...
    backgroundJobs.push(cron.schedule('*/5 * * * *', () => {
        reapStaleRuns({ maxIdleMinutes: 30, reason: '执行进程已中断' })
            .then((r) => { if (r.tasks > 0 || r.plans > 0)
            console.warn(`[autotest] 清理僵死运行态：任务 ${r.tasks} / 计划 ${r.plans}`); })
            .catch((e) => console.warn('[autotest] 僵死运行态清理失败：', e.message));
    }));
}
export function registerScheduledPlan(planId, cronExpr) {
    ...
    const job = cron.schedule(cronExpr, () => { executePlan(planId).catch((e) => console.error(`[autotest] 计划 #${planId} 执行失败：`, e.message)); });
    jobs.set(planId, job);
