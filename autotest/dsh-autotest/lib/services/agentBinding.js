 ...
 */
export async function resolveBinding(stage, libraryId) {
    const db = getDb();
    ...
    AND ((scope = 'library' AND library_id = ?) OR scope = 'global') ORDER BY (scope = 'library') DESC, id DESC`)
        .all(stage, libraryId ?? null);
    const hit = rows[0];
    if (hit) {
        return {
            stage,
            kind: String(hit.kind ?? 'prompt'),
            ...
            params: (() => { try {
                return JSON.parse(String(hit.params_json || '{}'));
            }
            catch {
                return {};
            } })(),
...
export async function listResolvedBindings(libraryId) {
    const db = getDb();
    const raw = await db.prepare('SELECT * FROM agent_bindings ORDER BY stage, scope').all();
...
export async function upsertBinding(b) {
    if (!stageDef(b.stage))
        throw Object.assign(new Error(`未知阶段：${b.stage}（可用阶段见 /agent/stages）`), { statusCode: 400 });
    if (b.scope === 'library' && !b.libraryId)
        throw Object.assign(new Error('按库绑定必须指定 libraryId'), { statusCode: 400 });
    if (b.kind === 'skill' && !String(b.skillPath ?? '').trim())
        throw Object.assign(new Error('绑定 Skill 必须给出 skillPath'), { statusCode: 400 });
    if (b.kind === 'prompt' && !b.promptId)
        throw Object.assign(new Error('绑定自写 Prompt 必须给出 promptId'), { statusCode: 400 });
    if (b.kind === 'external' && !String(b.externalCmd ?? '').trim())
        throw Object.assign(new Error('绑定外部 Agent 必须给出 externalCmd'), { statusCode: 400 });
    const db = getDb();
    const t = now();
    const existing = await db.prepare('SELECT id FROM agent_bindings WHERE stage = ? AND scope = ? AND library_id IS ?')
        .get(b.stage, b.scope, b.scope === 'library' ? b.libraryId ?? null : null);
    ...
    if (existing) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(b.stage, b.scope, b.scope === 'library' ? b.libraryId ?? null : null, b.promptId ?? null, String(b.skillPath ?? ''), String(b.model ?? ''), params, b.kind, String(b.externalCmd ?? ''), enabled, t, t);
    }
...
export function bindingTraceLine(b) {
    const parts = [`阶段 ${b.stage}`, `来源 ${b.source}`, `实现 ${AGENT_KIND_LABEL[b.kind]}`];
    if (b.promptId)
        parts.push(`prompt #${b.promptId}`);
    if (b.skillPath)
        parts.push(`skill ${b.skillPath}`);
    if (b.model)
        parts.push(`model ${b.model}`);
    if (b.externalCmd)
        parts.push(`cmd ${b.externalCmd}`);
    const keys = Object.keys(b.params);
    if (keys.length > 0)
        parts.push(`params ${JSON.stringify(b.params)}`);
    return parts.join(' · ');
}
// ---------- 外部 agent（命令行适配 + 最小 MCP 服务端） ----------
/** 外部 agent 的可用性探测（不实际调用，只看命令是否存在）。 */
export function externalCmdAvailable(cmd) {
    const first = String(cmd).trim().split(/\s+/)[0];
    if (!first)
        return { available: false, reason: '未配置命令' };
    const dirs = (process.env.PATH ?? '').split(path.delimiter);
    const exts = process.platform === 'win32' ? ['.cmd', '.exe', '.bat', '.ps1', ''] : [''];
    for (const d of dirs) {
        for (const ext of exts) {
            try {
                if (d && fs.existsSync(path.join(d, first + ext)))
                    return { available: true, reason: `${path.join(d, first + ext)}` };
            }
            catch { /* 忽略 */ }
        }
    }
