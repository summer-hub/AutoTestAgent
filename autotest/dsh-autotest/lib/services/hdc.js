...
export async function hdcAvailable() {
    try {
...
export async function tap(serial, x, y) {
    const mode = await detectDumpMode(serial);
    return mode === 'harmony'
...
export async function keyBack(serial) {
    const mode = await detectDumpMode(serial);
    if (mode === 'harmony') {
 ...
 */
export async function tailHilog(serial, lines = 12) {
    try {
...
async function verifyHilog(serial, keyword) {
    try {
        const { stdout } = await runHdc(['-t', serial, 'shell', 'hilog', '-x'], 20000);
        ...
        return { ok: false, log: `hilog 抓取失败：${e.message}` };
    }
}
...
export async function execShell(serial, shellArgs) {
...
async function runStep(serial, desc) {
    const t0 = Date.now();
    const fail = (log) => ({ ok: false, log, durationMs: Date.now() - t0 });
    const pass = (log) => ({ ok: true, log, durationMs: Date.now() - t0 });
    const d = desc.trim();
    try {
        ...
        if (/^(点击|单击|选择|选中|确认|打开|启动|切换|滚动|勾选|取消|删除)/.test(d)) {
            ...
            if (/^(打开|启动)/.test(d)) {
                ...
                try {
                    ...
                    return fail(`界面未找到「${kw}」且 aa start 失败：${e.message}`);
                }
            }
            return fail(`界面未找到「${kw}」`);
        }
        ...
        return fail(`执行异常：${e.message}`);
    }
}
/** 单步执行（带超时）。导出供 dryRun 逐步执行 —— 需在失败点抓取界面证据时不能用整批 executeCaseSteps。 */
export async function runStepWithTimeout(serial, desc, timeoutMs) {
    return Promise.race([
        runStep(serial, desc),
...
export async function executeCaseSteps(steps, serial, opts = {}) {
    const perStep = opts.perStepTimeoutMs ?? 30000;
    const logs = [`[hdc] 设备 ${serial} · 真实执行开始（${steps.length} 步）`];
    const results = [];
    let passed = true;
    ...
    if (opts.launch) {
        ...
        try {
            ...
            logs.push(`[hdc] 启动应用失败（继续执行步骤）：${e.message}`);
        }
    }
