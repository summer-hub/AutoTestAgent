...
import { ensureLibraryByRepoUrl, inspectRepo, pullRepo, recentChanges, refreshPackageInfo, updateRepo, workspaceDir, workspaceNotice, withGitSignal } from './gitRepo.js';
...
import { validateOracles } from './oracle.js';
...
export async function runTask(taskId, llm, signal) {
    const db = getDb();
    ...
    const claim = await db.prepare(`UPDATE tasks SET status='running', progress=10, error=NULL, updated_at=? WHERE id=? AND status <> 'running'`).run(now(), taskId);
...
async function execute(task, llm, signal) {
    const db = getDb();
...
async function repoTask(task, lib, signal) {
    const db = getDb();
...
export async function insertDraftCases(libraryId, rows, note, maxOverride) {
    const db = getDb();
    const t = now();
    const created = [];
    await db.transaction(async () => {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, '待确认', '未绑定', 1, ?, ?, ?, ?, ?, ?)`).run(libraryId, caseNo, r.name, r.source || 'AI 生成', r.precondition, JSON.stringify(r.steps), r.expected, r.apiSymbolId ?? null, r.scenarioKind ?? 'happy', r.priority ?? 'P1', JSON.stringify(r.oracles ?? []), t, t);
    ...
    for (const c of created) {
        try {
            ...
            if (r.ok)
                ...
                console.warn(`[hypium] ${c.caseNo} 未绑定：${r.reason}`);
        }
        catch (e) {
...
只输出一个 JSON 数组，每项：{ caseNo, name, precondition, steps[], expected }，不要解释。`;
    ...
    for (const o of offenders) {
        ...
        try {
            ...
            if (!rb.ok)
                console.warn(`[hypium] ${o.caseNo} 回灌后未绑定：${rb.reason}`);
        }
        catch { /* 绑脚本失败不影响用例修订 */ }
        void hlib;
        n++;
 ...
 */
async function dryRunAndRepair(taskId, llm, lib, created, packageName, signal) {
...
async function writeCases(task, lib, llm, signal) {
    // 确保仓库已下载到本地（真实代码上下文）
 ...
 */
async function exploreCases(task, lib, llm, signal) {
    const db = getDb();
    // 1. 设备与启动入口
    ...
    const pagesCompact = result.pages.map((p) => {
        ...
        const label = (c) => (c.text || c.desc).trim() || (c.id ? `#${c.id}` : `[${c.kind}]`);
        return {
            path: p.path.join(' → '),
...
export async function optimizeCaseById(caseId, llm) {
    ...
    const uncovered = await db.prepare(`SELECT s.name, s.kind, m.status, m.status_reason FROM coverage_matrix m
     JOIN api_symbols s ON s.id = m.symbol_id
     WHERE m.library_id = ? AND m.status <> 'covered' ORDER BY m.status, s.id LIMIT 20`).all(c.library_id);
...
${gapCtx}${knowledgeCtx}`;
    ...
    try {
        ...
        rebound = rb.ok;
        if (!rb.ok)
            console.warn(`[hypium] 优化后未重绑 ${c.case_no}：${rb.reason}`);
    }
    catch (e) {
    ...
    for (const a of additions) {
        ...
        if (steps.length === 0) {
            addRejected.push(`「${String(a.name ?? '').slice(0, 24)}」步骤为空`);
            continue;
        }
        const problems = validateOracles(a.oracles);
        if (problems.length > 0) {
            addRejected.push(`「${String(a.name ?? '').slice(0, 24)}」oracle 不达标：${problems.map((p) => p.reason).join('；').slice(0, 100)}`);
            continue;
        }
        addDrafts.push({
            name: String(a.name ?? '补缺用例').slice(0, 120), source: 'AI 生成',
            precondition: String(a.precondition ?? ''), steps, expected: String(a.expected ?? ''),
            scenarioKind: 'happy', priority: 'P1', oracles: a.oracles,
        });
    }
    const added = addDrafts.length > 0
        ...
        : [];
    if (addRejected.length > 0)
        console.warn(`[autotest] 补缺用例被拒：${addRejected.join('；')}`);
    return {
 ...
 */
async function generateDraftsForPlan(task, llm, plans, injectText, drafts, failures) {
    const bySymbol = new Map();
 ...
 */
async function integrateCases(task, lib, llm) {
    const db = getDb();
    const binding = await resolveBinding('case_draft', lib.id);
    await traceTask(task.id, '生效的 Agent 绑定', bindingTraceLine(binding));
    // ---------- ① 关联（初版用例 ↔ 接口） ----------
    ...
    SELECT c.id, c.case_no, c.name, c.steps, c.expected, c.oracle_json, c.scenario_kind, c.current_version,
     ...
     WHERE c.library_id = ? ORDER BY c.id`).all(lib.id);
    ...
    for (let i = 0; i < batch.length; i++) {
        ...
        try {
            const pagePath = String(JSON.parse(String(c.snapshot ?? '{}')).pagePath ?? '');
            ...
            const syms = (linksByCase.get(caseId) ?? [])
                ...
                .filter((x) => x.s)
                .map(({ l, s }) => {
                const params = JSON.parse(String(s.params_json || '[]'));
                const throws = JSON.parse(String(s.throws_json || '[]'));
            ...
            if (steps.length === 0) {
                failures.push(`${caseNo}：模型未返回步骤，保持初版不变`);
                continue;
            }
            ...
            const verdict = validateDraftsAgainstPages([{ name: String(res.data?.name ?? c.name), source: 'AI 生成', precondition: '', steps, expected: '', pagePath }], pagesByPath);
            if (verdict.dropped.length > 0) {
        ...
        scenario_kind = ?, priority = ?, source = ?, current_version = ?, updated_at = ? WHERE id = ?`)
                .run(name, precondition, JSON.stringify(steps), expected, JSON.stringify(res.data?.oracles), scenarioKind, priority, '遍历初版→正式', next, t, caseId);
            await db.prepare(`INSERT INTO case_versions (case_id, version, snapshot, change_note, author, author_type, created_at)
        VALUES (?, ?, ?, ?, 'AI 整合 Agent', 'ai', ?)`).run(caseId, next, JSON.stringify({
            ...
            failures.push(`${caseNo}：${e.message.slice(0, 140)}`);
        }
    ...
    if (upgraded.length > 0)
        await traceTask(task.id, '升级为正式用例', upgraded.join('\n'));
    if (failures.length > 0)
        await traceTask(task.id, '未升级（保持初版，原因逐条写明）', failures.slice(0, 20).join('\n'));
    ...
    if (gapSampled.selected.length > 0) {
        ...
        if (genFailures.length > 0)
            await traceTask(task.id, '补生成中的问题', genFailures.slice(0, 20).join('\n'));
...
async function toScript(task, lib, llm) {
    ...
    await traceTask(task.id, '生效的 Agent 绑定', bindingTraceLine(binding));
    const cases = await db.prepare(`SELECT id, case_no, name FROM cases WHERE library_id = ? AND script_status = '未绑定' ORDER BY id LIMIT 50`).all(lib.id);
    if (cases.length === 0)
