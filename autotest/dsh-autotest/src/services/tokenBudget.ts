...
export async function taskTokenUsage(taskId: number): Promise<TaskTokenUsage> {
  const row = await getDb().prepare(
    `SELECT COALESCE(SUM(tokens_in), 0) AS i, COALESCE(SUM(tokens_out), 0) AS o
       FROM agent_events WHERE task_id = ?`,
  ).get<{ i: number; o: number }>(taskId);
...
/** 预算上限（0 = 不限制）。 */
export function taskTokenBudget(): number {
 ...
 */
export async function ensureTaskTokenBudget(taskId?: number): Promise<void> {
  if (!taskId) return;
  ...
  if (used.total >= budget) {
    throw new Error(
      `任务 token 预算已用尽（已用 ${used.total} / 上限 ${budget}），已中止后续模型调用；可在系统配置调大 agent.maxTokensPerTask`,
    );
  }
}
