...
export function parseScriptSteps(script: string): string[] {
  const steps: string[] = [];
  for (const raw of script.split(/\r?\n/)) {
    ...
    if (/\.click\(\s*\)|\.tap\(\s*\)/.test(line)) {
      ...
      if (text) { steps.push(`点击 ${text[1]}`); continue; }
    }
    if (/inputText|input_text|sendKeys|\.input\(/.test(line)) {
      const input = line.match(/\(\s*['"]([^'"]+)['"]\s*\)/);
      if (input) { steps.push(`输入 ${input[1]}`); continue; }
    }
...
export function readBoundScript(libName: string, caseNo: string): string | null {
