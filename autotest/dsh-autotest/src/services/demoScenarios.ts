...
const STATUS_FROM_MARK: Record<string, ScenarioStatus> = {
  '✅': 'covered', '🔶': 'partial', '❌': 'uncovered',
};

/** 状态列 → 枚举。认不出返回 null（调用方必须报 warning，不许默认）。 */
export function parseStatus(mark: string): ScenarioStatus | null {
  const t = String(mark ?? '').trim();
  if (STATUS_FROM_MARK[t]) return STATUS_FROM_MARK[t];
  // 容忍 "✅ 完全覆盖" / "partial" 这类写法
  if (/完全覆盖|covered$/i.test(t) && !/未|not/i.test(t)) return 'covered';
  if (/部分覆盖|partial/i.test(t)) return 'partial';
  if (/未覆盖|not_covered|uncovered/i.test(t)) return 'uncovered';
  return null;
}

/** `2/2 (100%)` / `0/4 (0%)` → {covered:2,total:2}；解析失败返回 null。 */
export function parseRatio(cell: string): { covered: number; total: number } | null {
...
export function parseScenarioHeading(text: string): { no: string; name: string } | null {
...
export function extractInterfaces(line: string): string[] {
  const out: string[] = [];
  const re = /`([^`]+)`/g;
...
export function normalizeApi(raw: string): string {
...
export function parseScenarioList(md: string): {
  scenarios: ScenarioDef[];
  moduleMap: Array<{ module: string; apis: string[]; positive: string[]; negative: string[] }>;
  warnings: string[];
} {
  ...
  // ---- 功能模块映射表 ----
  const moduleMap: Array<{ module: string; apis: string[]; positive: string[]; negative: string[] }> = [];
  const noRe = /\b([PN]\d{1,3})\b/g;
  const collectNos = (cell: string): string[] => {
    const out: string[] = [];
    let m: RegExpExecArray | null;
    while ((m = noRe.exec(cell))) out.push(m[1]);
    noRe.lastIndex = 0;
    return out;
  };
  let inModuleTable = false;
  for (const line of lines) {
    if (/^#{2,6}\s*功能模块映射/.test(line)) { inModuleTable = true; continue; }
    if (inModuleTable && /^#{2,6}\s/.test(line)) { inModuleTable = false; }
    if (!inModuleTable) continue;
    const cells = splitRow(line);
    if (cells.length < 4 || isSeparatorRow(cells)) continue;
    if (/^模块$/.test(cells[0])) continue;
    moduleMap.push({
      module: cells[0],
  ...
  if (moduleMap.length === 0) warnings.push('场景清单里没有解析到「功能模块映射」表（模块统计会为空）');

  // ---- 场景块 ----
  const scenarios: ScenarioDef[] = [];
  // 映射表是**多对多**的（同一个场景可能同时列在多个模块行下），所以它不能单独决定归属。
  // 归属以场景块里的 `**模块**：X` 行为准；没有该行时才退回映射表，且多命中必须告警（不许静默选一个）。
  const moduleHits = new Map<string, string[]>();
  for (const m of moduleMap) {
    for (const no of [...m.positive, ...m.negative]) {
      const list = moduleHits.get(no) ?? [];
      if (!list.includes(m.module)) list.push(m.module);
      moduleHits.set(no, list);
    }
  }
  let cur: ScenarioDef | null = null;
  let curKindFromText: ScenarioKind | null = null;
  let curModuleExplicit = false;
  let inApis = false;
  const push = (): void => {
    if (!cur) return;
    if (!curKindFromText) warnings.push(`场景 ${cur.no} 缺少「场景类型」行，按编号前缀推断：${cur.kind === 'negative' ? '反向' : '正向'}`);
    if (cur.interfaces.length === 0) warnings.push(`场景 ${cur.no} 的「场景涉及的功能点」里没有解析到接口（接口覆盖率会算不出来）`);
  ...
  for (const line of lines) {
    ...
    if (/^\*\*场景涉及的功能点\*\*/.test(line.trim())) { inApis = true; continue; }
    if (inApis) {
      if (/^\*\*|^#{2,6}\s/.test(line.trim())) { inApis = false; continue; }
      if (/^[-*]\s/.test(line.trim()) && /对应接口/.test(line)) {
        for (const api of extractInterfaces(line).map(normalizeApi)) {
          if (!cur.interfaces.includes(api)) cur.interfaces.push(api);
        }
      }
    }
  }
  ...
  if (scenarios.length === 0) warnings.push('场景清单里没有解析到任何 `#### P01 ...` 形式的场景标题');
  const seen = new Set<string>();
  for (const s of scenarios) {
    if (seen.has(s.no)) warnings.push(`场景编号重复：${s.no}`);
    seen.add(s.no);
  }
...
export function parseCoverageReport(md: string): {
  ...
  const interfaces: InterfaceCheckRow[] = [];
  let mode: 'none' | 'scenario' | 'interface' = 'none';
  let baselineNote: string | null = null;

  for (const line of lines) {
    if (/^#{2,6}\s*(正向场景覆盖|反向场景覆盖)/.test(line)) { mode = 'scenario'; continue; }
    if (/^#{2,6}\s*接口维度逐条核对/.test(line)) { mode = 'interface'; continue; }
    if (/^#{2,6}\s/.test(line) && mode !== 'none') {
      // 场景覆盖表结束后进入别的章节；接口表只在它自己那节里认
      if (mode === 'interface') mode = 'none';
    }
    if (/^\s*>\s*基线 Demo/.test(line) || /\*\*基线 Demo（用户仓库/.test(line)) baselineNote = line.trim();
    ...
    if (mode === 'scenario') {
      ...
      if (!status) { warnings.push(`覆盖率报告里场景 ${cells[0]} 的覆盖状态列无法识别：${cells[2]}`); continue; }
      const ratio = parseRatio(cells[3] ?? '');
      if (!ratio) warnings.push(`场景 ${cells[0]} 的接口覆盖率列无法解析：${cells[3] ?? ''}`);
      judgements.push({
    ...
    if (mode === 'interface') {
      ...
      if (/^(接口|选项)/.test(apiCell) || /接口 \/ 选项/.test(apiCell)) continue;
      const api = extractInterfaces(apiCell).map(normalizeApi)[0] ?? normalizeApi(apiCell);
      if (!api || !/[A-Za-z]/.test(api)) continue;
      const verdictCell = (cells[1] ?? '').trim();
      const verdict: InterfaceCheckRow['verdict'] = verdictCell.startsWith('✅') ? 'executed'
  ...
  if (judgements.length === 0) warnings.push('覆盖率报告里没有解析到「场景覆盖详情」表（无法得到任何场景结论）');
  if (interfaces.length === 0) warnings.push('覆盖率报告里没有解析到「接口维度逐条核对」表（接口维度数字会为空）');
 ...
 */
export function mergeScenarioCoverage(scenarioMd: string, reportMd: string): {
  rows: ScenarioCoverageRow[];
  summary: ScenarioCoverageSummary;
  warnings: string[];
  modules: Array<{ module: string; apis: string[]; positive: string[]; negative: string[] }>;
} {
  ...
  const rows: ScenarioCoverageRow[] = [];
  const notes: string[] = [];
  ...
  if (notes.length > 0) warnings.push(`共 ${notes.length} 条结论与证据不一致，已写入对应行的 note`);
...
export function renderScenarioCoverageMarkdown(
  libName: string,
  rows: ScenarioCoverageRow[],
  summary: ScenarioCoverageSummary,
  warnings: string[],
...
export function scenarioDocPaths(libName: string): { dir: string; scenarioDoc: string; reportDoc: string } {
...
export function readScenarioDocs(libName: string): { scenarioMd: string; reportMd: string; missing: string[] } {
  const p = scenarioDocPaths(libName);
  const missing: string[] = [];
  let scenarioMd = '';
  let reportMd = '';
  if (fs.existsSync(p.scenarioDoc)) scenarioMd = fs.readFileSync(p.scenarioDoc, 'utf8');
  else missing.push(p.scenarioDoc);
  if (fs.existsSync(p.reportDoc)) reportMd = fs.readFileSync(p.reportDoc, 'utf8');
  else missing.push(p.reportDoc);
...
export async function loadScenarioCoverage(libraryId: number): Promise<ScenarioCoverageResult> {
  const db = getDb();
...
export async function syncScenarioCoverage(libraryId: number): Promise<ScenarioCoverageResult> {
  const r = await loadScenarioCoverage(libraryId);
  if (r.missing.length > 0 || r.rows === 0) return r;
  const db = getDb();
  const version = (await db.prepare('SELECT current_version FROM libraries WHERE id = ?')
    .get<{ current_version: string }>(libraryId))?.current_version ?? '';
  const t = now();
  await db.transaction(async () => {
    ...
    for (const s of r.scenarios) {
      await db.prepare(`INSERT INTO demo_scenario_coverage
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(libraryId, version, s.no, s.name, s.kind, s.module, s.status, s.apiCovered, s.apiTotal,
          s.evidence.slice(0, 500), s.gap.slice(0, 500), s.note.slice(0, 300),
          r.sources.scenarioDoc, r.sources.reportDoc, t, t);
    }
  });
  return r;
}
...
export async function readScenarioCoverageSnapshot(libraryId: number): Promise<{
  version: string; summary: ScenarioCoverageSummary; rows: number; updatedAt: string;
}> {
  const db = getDb();
  const rows = await db.prepare(`SELECT * FROM demo_scenario_coverage WHERE library_id = ? ORDER BY scenario_no`)
    .all<Record<string, unknown>>(libraryId);
  const mapped: ScenarioCoverageRow[] = rows.map((r) => ({
