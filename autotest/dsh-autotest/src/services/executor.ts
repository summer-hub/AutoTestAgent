...
只输出一个 JSON 数组，每项：{ index, name, precondition, steps[], expected, pagePath }，不要解释。`;
  try {
    const { data } = await llmJson<unknown>(llm, { system: sys, user: JSON.stringify(payload, null, 1), maxTokens: 4000, meta: { taskId, kind: 'repair_drafts' } });
...
${loadLessons(libName).map((l, i) => `${i + 1}. ${l}`).join('\n') || '（暂无）'}`;
    try {
      const { data } = await llmJson<{ pass?: boolean; issues?: unknown; revised?: unknown }>(llm, { system: sys, user, maxTokens: 6000, meta: { taskId, kind: 'review_refine' } });
...
只输出一个 JSON 数组，每项：{ caseNo, name, precondition, steps[], expected }，不要解释。`;
  ...
  try {
    const { data } = await llmJson<unknown>(llm, { system: sys, user: brief, maxTokens: 4000, meta: { taskId, kind: 'repair_dry_run' } });
    parsed = data;
  } catch (e) {
...
${loadLessons(lib.name).map((l, i) => `${i + 1}. ${l}`).join('\n') || '（暂无）'}`;
  ...
  for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
    try {
      const { data, text, provider, model } = await llmJson<unknown>(llm, attempt === 0
...
4. 来源固定为 AI 生成；每条用例必须带 pagePath 字段（值 = 该用例所属页面的 path 原文）。`;
  ...
  const processShard = async (si: number): Promise<{ si: number; drafts: DraftCase[]; fail: string | null }> => {
...
${lessonsText}`;
    try {
      const { data, text, provider, model } = await llmJson<unknown>(llm, { system: sys, user, maxTokens: 6000, meta: spanMeta(task, 'explore_cases') });
  ...
  const results: Array<{ si: number; drafts: DraftCase[]; fail: string | null }> = [];
...
${gapCtx}${knowledgeCtx}`;
  ...
  for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
    try {
      const { data } = await llmJson<unknown>(llm, attempt === 0
...
任务要求：${task.input || '根据最新版本变更更新上述用例（版本自动递增）。'}`;
  ...
  for (let attempt = 0; attempt < 2 && !updates; attempt++) {
    try {
      const { data, text, provider, model } = await llmJson<Array<{ caseNo: string; name?: string; expected?: string; changeNote?: string }>>(llm, attempt === 0
...
async function matrixCases(task: TaskRow, lib: RepoLib & { description: string }, llm: LlmCall): Promise<string> {
  ...
  if (sampled.selected.length === 0) return '没有可生成的用例计划（所有符号都被判定为不可测）。';

  // 按符号分片调用：每片独立、单片失败只重试该片，避免一次超长调用被截断
  const bySymbol = new Map<number, typeof sampled.selected>();
...
async function generateDraftsForPlan(
  ...
  for (const [symbolId, cases] of bySymbol) {
    ...
    try {
      const res = await llmJson<{ cases?: Array<{ name?: string; precondition?: string; steps?: string[]; expected?: string }> }>(
        llm,
        {
