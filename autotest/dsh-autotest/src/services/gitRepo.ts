...
export async function reconcileRepos(): Promise<number> {
  ...
  for (const l of libs) {
    ...
    if (!hasRepo && (row.last_commit || row.last_synced_at)) {
      ...
      try { await refreshPackageInfo({ id: l.id, name: l.name }); } catch { /* 忽略 */ }
    }
  }
  return changed;
}
...
export async function ensureLibraryByRepoUrl(url: string): Promise<RepoLib> {
  ...
  const existing = await db.prepare('SELECT * FROM libraries WHERE repo_url = ? OR repo_url = ?').get<RepoLib>(normalized, trimmed);
