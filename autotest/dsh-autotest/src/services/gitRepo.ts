...
export async function reconcileRepos(): Promise<number> {
  const db = getDb();
  const libs = await db.prepare('SELECT id, name, repo_url, repo_subpath FROM libraries').all<{ id: number; name: string; repo_url: string; repo_subpath: string }>();
  let changed = 0;
  for (const l of libs) {
    ...
    const row = await db.prepare('SELECT last_commit, last_synced_at, package_name FROM libraries WHERE id = ?')
      .get<{ last_commit: string; last_synced_at: string | null; package_name: string }>(l.id);
    if (!row) continue;
    if (!hasRepo && (row.last_commit || row.last_synced_at)) {
      ...
      try { await refreshPackageInfo(l); } catch { /* 忽略 */ }
    }
  }
  return changed;
}
 ...
 */
export async function migrateRepoSubpaths(): Promise<number> {
  const db = getDb();
  const rows = await db.prepare(`SELECT id, name, repo_url, repo_subpath FROM libraries WHERE repo_url LIKE '%/tree/%'`)
    .all<{ id: number; name: string; repo_url: string; repo_subpath: string }>();
  let changed = 0;
  for (const r of rows) {
    const { repoUrl, subpath } = splitRepoUrl(r.repo_url);
    if (!repoUrl || repoUrl === r.repo_url) continue;   // 没拆出东西就不动
 ...
 */
export function splitRepoUrl(url: string): { repoUrl: string; subpath: string; branch: string } {
  const raw = String(url || '').trim();
  const m = /^(.*?)\/tree\/([^/]+)(?:\/(.*?))?\/?$/.exec(raw);
  const repoUrl = normalizeRepoUrl(raw);
  if (!m) return { repoUrl, subpath: '', branch: '' };
 ...
 */
export function normalizeSubpath(sub: string): string {
  let s = String(sub || '');
  try {
...
export function repoRootDir(repoUrl: string): string {
...
function repoRootDirOwnered(repoUrl: string): string {
  const { repoUrl: root } = splitRepoUrl(repoUrl);
...
function sameRepoNameOnly(a: string, b: string): boolean {
  const tail = (u: string): string => u.replace(/\.git$/, '').split(/[/\\]/).filter(Boolean).pop() ?? '';
  const ta = tail(a);
  return !!ta && ta === tail(b);
}

/** 库目录 = 仓库根 + 库子目录；未配仓库地址时退回按库名定位（历史行为）。 */
export function repoDirFor(lib: { name: string; repo_url?: string; repo_subpath?: string }): string {
  const url = String(lib.repo_url ?? '').trim();
  if (!url) return path.join(workspaceDir(), 'repos', repoName(lib.name));
 ...
 */
export function resolveRepoRootDir(repoUrl: string, legacyNames: string[] = [], expectedCommit = ''): string {
  ...
  if (fs.existsSync(path.join(canonical, '.git'))) {
    ...
    if (!remote || !target || remote === target) return canonical;
    if (sameRepoNameOnly(remote, target)) {
      ...
      try {
        ...
        console.warn(`[autotest] 更新 remote 失败（继续用本地克隆）：${(e as Error).message}`);
      }
      return canonical;
    }
  ...
  for (const legacy of legacyNames) {
    ...
    try {
      ...
      console.warn(`[autotest] 仓库目录改名失败（继续就地使用 ${dir}）：${(e as Error).message}`);
      return dir;
    }
  }
  return canonical;
...
export function repoRootForLib(lib: RepoLib | { name: string; repo_url?: string; repo_subpath?: string; last_commit?: string }): string {
  const url = String(lib.repo_url ?? '').trim();
  return url
 ...
 */
export function inspectRepo(lib: { name: string; repo_url?: string; repo_subpath?: string; last_commit?: string }): RepoInspect {
  const url = String(lib.repo_url ?? '').trim();
  const dir = url
  ...
  if (!fs.existsSync(path.join(dir, '.git')) && !fs.existsSync(path.join(dir, 'AppScope')) && !fs.existsSync(path.join(dir, 'entry'))) return result;
  const candidates = [
...
export function recentChanges(lib: RepoLib): string[] {
  ...
  const args = (base: string[]): string[] => (prefix ? [...base, '--', prefix] : base);
  try {
...
export async function ensureLibraryByRepoUrl(url: string): Promise<RepoLib> {
  ...
  const existing = await db.prepare('SELECT * FROM libraries WHERE repo_url = ? OR repo_url = ? OR repo_url = ?')
    .get<RepoLib>(trimmed, normalized, normalized || trimmed);
 ...
 */
async function describeVersion(dir: string, fallback: string, subpath = ''): Promise<string> {
  if (subpath) {
    try {
      const short = await runGit(['log', '-1', '--format=%h', '--', subpath], dir);
      if (short) return `dev-${short}`;
    } catch { /* 落到 fallback */ }
    return fallback;
  }
  try {
...
export async function refreshPackageInfo(lib: { id: number; name: string; repo_url?: string; repo_subpath?: string }): Promise<{ packageName: string; mainAbility: string }> {
...
export async function pullRepo(lib: RepoLib): Promise<RepoResult> {
  ...
  const summary = action === 'clone'
    ...
    : `已更新 ${lib.name}${where}（${branch} @ ${commit.slice(0, 8)}），当前版本 ${version}${
