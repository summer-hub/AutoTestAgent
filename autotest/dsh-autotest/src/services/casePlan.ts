...
export const SCENARIO_ORDER: Scenario[] = ['happy', 'empty', 'boundary', 'bigdata'];
...
function paramText(s: PlanSymbolInput): string {
 ...
 */
export function computeApplicability(s: PlanSymbolInput): Applicability {
  const fit: Record<Scenario, boolean> = { happy: true, empty: false, boundary: false, bigdata: false };
  ...
  if (optionalParams.length > 0) emptyWhy.push(`${optionalParams.length} 个可选参数（不传即空值路径：${optionalParams.map((p) => p.name).join('/')}）`);
  if (containerParams.length > 0) emptyWhy.push(`参数含容器/字符串/对象类型（可传空串/空数组/空对象：${containerParams.map((p) => `${p.name}:${p.type || '未标注'}`).join('/')}）`);
  if (setterMethods.length > 0) emptyWhy.push(`有 setter 类接口（可设空值：${setterMethods.slice(0, 4).join('/')}）`);
  ...
  const boundaryWhy: string[] = [];
  if (throwsCount > 0) boundaryWhy.push(`声明了 ${throwsCount} 类异常（${s.throws.map((t) => t.type || 'Error').join('/')}）`);
  if (numberParams.length > 0) boundaryWhy.push(`含数值型参数（有取值范围/越界空间：${numberParams.map((p) => p.name).join('/')}）`);
  if (stateful.length > 0) boundaryWhy.push(`有状态机/生命周期方法（可重复调用、逆序调用、释放后调用：${stateful.slice(0, 5).join('/')}）`);
  if (s.params.some((p) => /(schema|object|json)/i.test(p.type))) boundaryWhy.push('参数为结构体/JSON：可构造类型错误与非法结构');
  ...
  const bigWhy: string[] = [];
  if (sizeParams.length > 0) bigWhy.push(`参数含尺寸/数量/内容（可放大：${sizeParams.map((p) => p.name).join('/')}）`);
  if (mediaSymbol) bigWhy.push(`接口名/方法名涉及集合·媒体·渲染（${[...[s.name], ...s.methods].filter((x) => MEDIA_WORD.test(x)).slice(0, 4).join('/')}）`);
 ...
 */
export function computeNegExtra(s: PlanSymbolInput): NegExtra {
  const reasons: string[] = [];
  const boundary = s.throws.length + (s.methods.some((m) => STATEFUL_METHOD.test(m)) ? 1 : 0);
  if (s.throws.length > 0) reasons.push(`每条声明异常各一条（${s.throws.map((t) => t.type || 'Error').join('/')}）`);
  if (s.methods.some((m) => STATEFUL_METHOD.test(m))) reasons.push('状态机多一条：重复调用 / 未初始化即调用');
  const empty = s.params.filter((p) => p.optional).length >= 2 ? 1 : 0;
  if (empty > 0) reasons.push('多个可选参数时补一条"全部不传"');
  return { empty, boundary, reasons };
}

/** 触发器：同一接口在 demo 里可能有多个入口页面，按入口展开（设计 §6.4 的可选项）。 */
export function computeTriggers(s: PlanSymbolInput): Array<{ pagePath: string; control: string }> {
  const seen = new Map<string, { pagePath: string; control: string }>();
  ...
  if (seen.size === 0) seen.set('', { pagePath: '', control: '' });
  return [...seen.values()];
}

function priorityOf(scenario: Scenario): Priority {
  if (scenario === 'happy') return 'P0';
  if (scenario === 'bigdata') return 'P2';
  return 'P1';
}

function inputPlanOf(s: PlanSymbolInput, scenario: Scenario): string {
...
function scenarioInputHint(scenario: Scenario): string {
  switch (scenario) {
...
function assertionPlanOf(s: PlanSymbolInput, scenario: Scenario): string {
 ...
 */
export function planSymbolCases(s: PlanSymbolInput, opts: { maxTriggersPerSymbol?: number; maxCasesPerSymbol?: number } = {}): PlannedCase[] {
  ...
  const push = (scenario: Scenario, because: string, triggerIndex: number): void => {
  ...
  for (let i = 1; i < triggers.length; i++) {
    if (triggers[i].pagePath) push('happy', `同一接口按不同入口分别覆盖（入口 ${i + 1}）`, i);
  }
  if (fit.empty) {
    push('empty', reasons.empty, 0);
    for (let i = 0; i < neg.empty; i++) push('empty', `负向补充：${neg.reasons.find((r) => r.includes('全部不传')) ?? '可选参数组合'}`, 0);
  }
  if (fit.boundary) {
    push('boundary', reasons.boundary, 0);
    for (const t of s.throws) push('boundary', `负向补充：声明异常 ${t.type || 'Error'}${t.doc ? `（${t.doc}）` : ''} 各一条`, 0);
    if (neg.boundary > s.throws.length) push('boundary', '负向补充：重复调用 / 未初始化即调用（状态机）', 0);
  }
  if (fit.bigdata) push('bigdata', reasons.bigdata, 0);
...
export function summarizePlans(
  perSymbol: Array<{ symbolId: number; name: string }>,
  plans: PlannedCase[],
): Pick<LibraryPlan['summary'], 'planned' | 'byScenario' | 'byPriority' | 'byKind' | 'perSymbolBreakdown'> {
  const byScenario = { happy: 0, empty: 0, boundary: 0, bigdata: 0 } as Record<Scenario, number>;
  const byPriority = { P0: 0, P1: 0, P2: 0 } as Record<Priority, number>;
  ...
  const perSymbolBreakdown = perSymbol.map((p) => {
    ...
    const countOf = (sc: Scenario): number => mine.filter((x) => x.scenario === sc).length;
...
export interface LibraryPlan {
  ...
  plans: PlannedCase[];
  perSymbol: Array<{
    symbolId: number; name: string; kind: string;
...
export async function planLibraryCases(libraryId: number, opts: { maxTriggersPerSymbol?: number; maxCasesPerSymbol?: number } = {}): Promise<LibraryPlan> {
  const db = getDb();
  ...
  const matrix = await db.prepare('SELECT symbol_id, status, risk_flags, evidence_json FROM coverage_matrix WHERE library_id = ?')
    .all<{ symbol_id: number; status: string; risk_flags: string; evidence_json: string }>(libraryId);
  const matrixBySymbol = new Map(matrix.map((m) => [m.symbol_id, m]));

  const plans: PlannedCase[] = [];
  const perSymbol: LibraryPlan['perSymbol'] = [];
  const skippedSymbols: Array<{ name: string; kind: string; reason: string }> = [];
  ...
  for (const s of symbolRows) {
    ...
    const ev = evidence ? JSON.parse(evidence.evidence_json || '{}') as { deviceControls?: string[]; paramPoints?: Array<{ pagePath: string; name: string; sourceFile: string; sourceLine: number }> } : {};
 ...
 */
export function samplePlan(plans: PlannedCase[], budget: number): {
  ...
  for (const s of skipped) byPrio[s.priority] = (byPrio[s.priority] ?? 0) + 1;
  return {
