...
function suiteName(libName: string): string {
...
function buildTestMethods(pages: ExploredPage[]): string {
  ...
  for (const page of pages) {
    ...
    for (const c of page.controls.slice(0, 5)) {
      ...
      if (kw) verifyLines.push(`        self.driver.find_component(BY.text("${esc(kw)}"))`);
    }
    methods.push(
...
/** 生成单个测试套件 Python 脚本。 */
export function generateHypiumScript(
  lib: { name: string; packageName: string },
  pages: ExploredPage[],
): string {
...
function userConfig(serial: string): string {
  return [
...
function mainPy(suite: string): string {
  return [
...
export function writeHypiumProject(
  lib: { name: string; packageName: string },
  pages: ExploredPage[],
  serial: string,
): HypiumProject {
