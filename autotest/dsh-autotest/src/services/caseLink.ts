...
export const BASIS_LABEL: Record<LinkBasis, string> = {
  explicit: '生成时指定接口',
...
export function isTrustedLink(l: { confidence: LinkConfidence }): boolean {
...
export function resolveCaseRoute(pagePath: string, traversal: TraversalEvidence | null): { route: string; how: string } {
  const p = String(pagePath || '').trim();
  if (!p || !traversal) return { route: '', how: '' };
  const exact = traversal.breadcrumbToRoute.get(p);
  if (exact) return { route: exact, how: '面包屑完全匹配' };
  ...
  if (last) {
    for (const [crumb, route] of traversal.breadcrumbToRoute) {
      ...
      if (crumbLast && crumbLast === last) return { route, how: `末段页面名「${last}」匹配` };
    }
  }
  // 最后再试：用例直接写了路由名
  if (traversal.routes.has(p)) return { route: p, how: '直接写的路由名' };
  return { route: '', how: '' };
}

/** 名称命中：符号名或其方法名出现在用例文本里（要求 ≥3 字符，避免 validate 这类通用词误伤短名）。 */
export function nameHits(text: string, symbolName: string, methods: string[]): string[] {
  ...
  for (const c of candidates) {
    ...
    if (re.test(hay)) hits.push(c.trim());
  }
  return hits;
}
...
export function inferLinks(
  ...
  const put = (sym: SymbolForLink, basis: LinkBasis, confidence: LinkConfidence, detail: string): void => {
    const prev = out.get(sym.id);
    const rank: Record<LinkBasis, number> = { explicit: 4, page: 3, name: 2, manual: 5 };
    if (prev && rank[prev.basis] >= rank[basis]) return;
  ...
  for (const s of symbols) {
    ...
    if (isTypeSymbol(s.kind)) continue; // 类型符号没有 UI 行为，不做推断关联
    ...
    if (resolved.route && s.demoCallPages.includes(resolved.route)) {
      ...
      put(s, 'page', 'high',
        `用例页面「${c.pagePath}」→ 路由 ${resolved.route}（${resolved.how}），该接口正是在此页被调用`
...
function controlsOf(c: CaseForLink, deviceControls: string[]): string[] {
  if (deviceControls.length === 0) return [];
  const norm = (s: string): string => s.replace(/\s+/g, '').toLowerCase();
  const pool = new Set(deviceControls.map(norm));
  const refs = new Set<string>();
  for (const step of c.steps) for (const m of step.matchAll(/「([^「」]{1,40})」/g)) refs.add(norm(m[1]));
 ...
 */
export async function linkCasesForLibrary(libraryId: number): Promise<LinkRunResult> {
  const db = getDb();
  ...
  const symbolRows = await db.prepare('SELECT id, name, kind, methods_json FROM api_symbols WHERE library_id = ? AND library_version = ? ORDER BY id')
    .all<{ id: number; name: string; kind: string; methods_json: string }>(libraryId, version);
  const callAssets = await db.prepare(`SELECT name, page_path FROM demo_assets WHERE library_id = ? AND library_version = ? AND kind = 'call'`)
    .all<{ name: string; page_path: string }>(libraryId, version);
  ...
  const caseRows = await db.prepare(`
    SELECT c.id, c.case_no, c.name, c.steps, c.expected, c.api_symbol_id,
     ...
     WHERE c.library_id = ?`)
    .all<{ id: number; case_no: string; name: string; steps: string; expected: string; api_symbol_id: number | null; snapshot: string | null }>(libraryId);

  const traversal = loadTraversalEvidence(lib.name);
  const symbols: SymbolForLink[] = symbolRows.map((s) => {
    const pages = callAssets.filter((a) => a.name === s.name).map((a) => String(a.page_path || '')).filter(Boolean);
    const controls = new Set<string>();
    for (const p of pages) for (const ctl of traversal?.routes.get(p)?.controls ?? []) controls.add(ctl);
    return {
      id: Number(s.id), name: String(s.name), kind: String(s.kind),
      methods: JSON.parse(String(s.methods_json || '[]')) as string[],
  ...
  const t = now();
  const candidates: Array<LinkCandidate & { caseNo: string }> = [];
  for (const c of caseRows) {
    let pagePath = '';
    try { pagePath = String((JSON.parse(String(c.snapshot ?? '{}')) as { pagePath?: string }).pagePath ?? ''); } catch { pagePath = ''; }
    const row: CaseForLink = {
      id: Number(c.id), caseNo: String(c.case_no), name: String(c.name),
      steps: JSON.parse(String(c.steps || '[]')) as string[],
    ...
    for (const l of inferLinks(row, symbols, traversal)) candidates.push({ ...l, caseNo: row.caseNo });
  }
  ...
  const manual = await db.prepare(`SELECT id, case_id, symbol_id, confidence, detail FROM case_symbol_links WHERE library_id = ? AND basis = 'manual'`)
    .all<{ id: number; case_id: number; symbol_id: number; confidence: string; detail: string }>(libraryId);
  ...
  await db.transaction(async () => {
    ...
    for (const l of candidates) {
      if (manualKeys.has(`${l.caseId}:${l.symbolId}`)) continue; // 人工确认的不动
  ...
  for (const l of candidates) byBasis[l.basis] = (byBasis[l.basis] ?? 0) + 1;
  const linkedCaseIds = new Set(candidates.map((c) => c.caseId));
  for (const m of manual) linkedCaseIds.add(m.case_id);
  return {
    libraryId,
...
export async function listLinks(libraryId: number, opts: { includeWeak?: boolean } = {}): Promise<CaseLinkRow[]> {
  const db = getDb();
  const rows = await db.prepare(`
    SELECT l.case_id, l.symbol_id, l.basis, l.confidence, l.detail,
     ...
     WHERE l.library_id = ?
     ORDER BY l.case_id, l.symbol_id`).all<Record<string, unknown>>(libraryId);
  return rows
...
export async function loadLinksBySymbol(libraryId: number): Promise<Map<number, Array<{ caseId: number; caseNo: string; basis: string; confidence: string }>>> {
  const rows = await listLinks(libraryId, { includeWeak: true });
  const map = new Map<number, Array<{ caseId: number; caseNo: string; basis: string; confidence: string }>>();
  for (const r of rows) {
...
export async function setManualLink(caseId: number, symbolId: number, linked: boolean): Promise<{ ok: boolean; message: string }> {
  const db = getDb();
  const c = await db.prepare('SELECT id, library_id FROM cases WHERE id = ?').get<{ id: number; library_id: number }>(caseId);
  if (!c) return { ok: false, message: '用例不存在' };
  const s = await db.prepare('SELECT id FROM api_symbols WHERE id = ?').get<{ id: number }>(symbolId);
  if (!s) return { ok: false, message: '接口符号不存在' };
  const t = now();
  if (!linked) {
...
export async function linkCaseToSymbols(caseId: number, symbolIds: number[], detail: string): Promise<number> {
  if (symbolIds.length === 0) return 0;
  const db = getDb();
  const c = await db.prepare('SELECT library_id FROM cases WHERE id = ?').get<{ library_id: number }>(caseId);
  if (!c) return 0;
  const t = now();
  let n = 0;
  for (const sid of [...new Set(symbolIds)]) {
    const manual = await db.prepare(`SELECT id FROM case_symbol_links WHERE case_id = ? AND symbol_id = ? AND basis = 'manual'`)
      .get<{ id: number }>(caseId, sid);
    if (manual) continue; // 人工确认优先，不覆盖
    const exists = await db.prepare('SELECT id FROM case_symbol_links WHERE case_id = ? AND symbol_id = ?').get<{ id: number }>(caseId, sid);
    if (exists) {
 ...
 */
export function isDraftCase(oracleJson: string | null | undefined, validate: (v: unknown) => Array<{ reason: string }>): boolean {
  let oracles: unknown;
  try {
  ...
  if (!Array.isArray(oracles) || oracles.length === 0) return true;
  return validate(oracles).length > 0;
}

/** 关联情况的人类可读报告（任务轨迹与接口返回共用）。 */export function renderLinkReport(lib: RepoLib | { name: string }, r: LinkRunResult): string {
  const basisText = Object.entries(r.byBasis).map(([k, v]) => `${BASIS_LABEL[k as LinkBasis] ?? k} ${v} 条`).join(' · ') || '（无）';
  return [
