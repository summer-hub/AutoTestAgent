 ...
 */
export function findHeader(rows: unknown[][]): { row: number; nameCol: number; urlCol: number } | null {
  for (let i = 0; i < Math.min(rows.length, 10); i++) {
    ...
    if (nameCol >= 0 && urlCol >= 0 && nameCol !== urlCol) return { row: i + 1, nameCol, urlCol };
  }
  return null;
}
 ...
 */
export function parseLibrarySheet(rows: unknown[][]): ParseResult {
...
export function diffLibrarySheet(entries: SheetEntry[], existing: ExistingLibrary[], problems: SheetProblem[] = []): LibrarySyncPlan {
  const byName = new Map(existing.map((l) => [l.name, l]));
  const plan: LibrarySyncPlan = { added: [], updated: [], unchanged: [], dbOnly: [], problems: [...problems] };
 ...
 */
export function resolveSheetPath(explicit?: string): { file: string; exists: boolean; tried: string[] } {
  ...
  for (const c of candidates) {
    if (fs.existsSync(c)) return { file: c, exists: true, tried: candidates };
  }
  return { file: candidates[0], exists: false, tried: candidates };
}

/** 读取 xlsx 并解析出库清单（不含 DB 交互）。 */
export function readSheet(file: string): ParseResult & { file: string } {
  ...
  if (!ws) throw Object.assign(new Error(`表里没有任何工作表：${file}`), { statusCode: 400 });
  const rows = XLSX.utils.sheet_to_json<unknown[]>(ws, { header: 1, raw: false, defval: '' });
  const parsed = parseLibrarySheet(rows);
  if (parsed.entries.length === 0) {
    throw Object.assign(
      new Error(`表里没有解析出任何库（工作表 ${sheetName}，共 ${rows.length} 行）。请确认表头含「三方库名称」与「URL」两列。`),
      { statusCode: 400 },
    );
  }
  return { ...parsed, file };
...
async function loadExisting(): Promise<ExistingLibrary[]> {
  return await getDb().prepare(
    `SELECT l.id, l.name, l.repo_url, l.repo_subpath, l.package_name,
 ...
 */
export async function syncLibrariesFromSheet(opts: { file?: string; apply?: boolean } = {}): Promise<{
  file: string; total: number; plan: LibrarySyncPlan; applied: boolean; header: ParseResult['header'];
}> {
  ...
  if (!apply) return { file, total: parsed.entries.length, plan, applied: false, header: parsed.header };
 ...
 */
export async function exportLibrariesSheet(outFile?: string): Promise<{ file: string; rows: number }> {
  const rows = await getDb().prepare(
    `SELECT l.name, l.repo_url, l.repo_subpath, l.package_name, l.main_ability, l.status,
