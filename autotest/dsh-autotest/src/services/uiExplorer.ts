...
import { dumpMeta, execShell, findKeyword, inputText, isSystemBundle, keyBack, launchArgs, listTargets, parseDump, parseNodes, resolveMainAbility, screenSize, tap, uiDump, withDeviceSignal, type UiNode } from './hdc.js';
import {
  budgetCheck, classifyControl, CoverageTracker, coverageHealth, dedupKey, isClickCandidate,
  isInteractive, nodeIdentity, nodeLabel, pageSignature, type ControlKind, type CoverageReport, type ExploreBudget,
} from './uiModel.js';
...
function collectControls(
  ...
  const interactive: RawNode[] = [];
  const labels: RawNode[] = [];
  const taken = new Set<string>();
  for (const n of nodes) {
    if (!n.bounds) continue;
    const key = nodeIdentity(n);
    if (n.y <= statusBarY) {                       // 状态栏/导航区
      if (isInteractive(n)) tracker.skip(key, 'offScreenContent');
      continue;
    }
    if (isClickCandidate(n, screen)) {
      if (taken.has(key)) { tracker.skip(key, 'duplicate'); continue; }
      taken.add(key);
      interactive.push(n);
      tracker.markCollected(n);
      continue;
    }
    if (n.clickable || n.longClickable) { tracker.skip(key, 'fullScreenContainer'); continue; }
    if (isInteractive(n)) { tracker.skip(key, 'notClickable'); continue; }
    if ((n.text || n.desc).trim()) labels.push(n); // 纯展示文本：入清单供语义参考，但不点击
  }
...
/**
 * 遍历使用的节点类型 = hdc 解析出的完整 UiNode（含 clickable/id/checkable 等交互标志）。
 * 旧版这里是一份只有 text/desc/坐标的精简结构，把交互标志全丢了，导致
 * "候选必须有文本" 这个错误前提（真机上恰好把可点控件全丢掉）。
 */
type RawNode = UiNode;
...
async function exploreAppInner(
  serial: string,
  packageName: string,
  opts: ExploreOpts = {},
): Promise<ExploreResult> {
  ...
  const budget: ExploreBudget = { maxPages, maxMinutes, maxClicksPerPage, maxSwipePerPage };
  ...
  let cachedStartArgs: string[] | null = null;
  const startArgsFor = async (bundle: string): Promise<string[]> => {
    if (cachedStartArgs && cachedStartBundle === bundle) return cachedStartArgs;
    ...
    if (given) {
      ...
      if (ab) op('解析入口 Ability', `${bundle} → ${ab}`);
  ...
  const launchApp = async (): Promise<boolean> => {
  ...
  const dumpCurrent = async (): Promise<{ nodes: RawNode[]; screen: { w: number; h: number }; sw: number; meta: ReturnType<typeof dumpMeta>; parsed: boolean; parseReason?: string }> => {
  ...
  while (queue.length > 0) {
    ...
    if (opts.signal?.aborted) { stopReason = '任务已取消'; op('遍历中断', '任务已取消'); break; }
    const b = budgetCheck({ pages: pages.length, clicksOnPage: 0, steps: tracker.steps, elapsedMs: Date.now() - t0 }, budget);
    if (b.exhausted) { stopReason = b.reason; op('预算耗尽停止', b.reason); break; }
    if (guard++ > Math.max(200, maxPages * 8)) { stopReason = 'guard'; op('遍历步数保护触发', `${guard}`); break; }
    const cur = queue.shift()!;
    if (cur.depth > maxDepth) { op('超过深度安全上限，跳过展开', `${cur.path.join('→')}（深度 ${cur.depth}）`); continue; }
    ...
    while (vp <= maxSwipePerPage) {
      ...
      for (const n of nodesVp) {
        ...
        if (n.y <= yMinVp) { if (isInteractive(n)) tracker.skip(key, 'offScreenContent'); continue; }
        if (!isClickCandidate(n, scrVp)) {
          if (n.clickable || n.longClickable) tracker.skip(key, 'fullScreenContainer');
          else if (isInteractive(n)) tracker.skip(key, 'notClickable');
          else if (!(n.text || n.desc).trim()) tracker.skip(key, 'notInteractive');
          continue;
        }
        // 导航类控件不扩展（点了会离开当前页，由返回手势统一管理）
        if (/^(返回|back|上一页|关闭|取消|X$)/i.test((n.text || n.desc).trim())) { tracker.skip(key, 'navigation'); continue; }
        // 纯滚动容器不按点击处理（由视口翻屏循环覆盖）
        if (classifyControl(n) === 'scroll') { tracker.skip(key, 'notClickable'); continue; }
        const dk = dedupKey(n, vp);
        if (seenCandidates.has(dk)) { tracker.skip(key, 'duplicate'); continue; }
        ...
        if (kind === 'dropdown') {
          ...
          if (opened.parsed) {
            ...
            for (const n of opened.nodes) {
              if (!n.bounds || n.y <= yMinVp) continue;
              if (!isClickCandidate(n, opened.nodes.length ? scrVp : scrVp)) continue;
              const k = nodeIdentity(n);
              if (inventoryKeys.has(k)) continue;
              inventoryKeys.add(k);
              inventory.push(n);
        ...
        if (kind === 'checkbox' || kind === 'switch') {
          ...
          if (after.parsed) {
            ...
            for (const n of after.nodes) {
              if (!n.bounds || n.y <= yMinVp) continue;
              if (!(n.text || n.desc).trim() && !isInteractive(n)) continue;
              const k = nodeIdentity(n);
              if (inventoryKeys.has(k)) continue;
              inventoryKeys.add(k);
              inventory.push(n);
        ...
        if (kind === 'input') {
          ...
          try {
            ...
            if (after.parsed) {
              ...
              for (const n of after.nodes) {
                if (!n.bounds || n.y <= yMinVp) continue;
                if (!(n.text || n.desc).trim()) continue;
                const k = nodeIdentity(n);
                if (inventoryKeys.has(k)) continue;
                inventoryKeys.add(k);
                inventory.push(n);
            ...
            op('输入探针失败', `${(e as Error).message.slice(0, 120)}`);
          }
    ...
    // 滑回顶部，下一轮从已知位置开始
    if (vp > 0) op('下滑回顶', `${vp} 屏 · 本页累计控件清单 ${Math.min(inventory.length, controlsPerPage)} 条`);
    for (let i = 0; i < vp; i++) { await swipePage(serial, 'down'); tracker.step(); }
    ...
    if (selfPage && inventory.length > 0) {
      const ctlKey = (c: ExploredControl): string => c.id || `${c.text}|${c.desc}|${c.x},${c.y}`;
      const byKey = new Map<string, ExploredControl>();
      for (const c of selfPage.controls) byKey.set(ctlKey(c), c);
      const ordered = [
        ...inventory.filter((n) => isClickCandidate(n, selfPage.screen)),
        ...inventory.filter((n) => !isClickCandidate(n, selfPage.screen)),
      ].map(toControl);
      for (const c of ordered) if (!byKey.has(ctlKey(c))) byKey.set(ctlKey(c), c);
      ...
      for (const n of inventory) tracker.markCollected(n);
      if (vp > 0) {
        selfPage.scrolls = vp;
  ...
  if (!health.ok) {
    for (const w of health.warnings) console.warn(`[explore] 覆盖率告警：${w}`);
