...
export function loadTraversalEvidence(libName: string): TraversalEvidence | null {
  ...
  const breadcrumbToRoute = new Map<string, string>();
  for (const p of report.pages ?? []) {
    ...
    if (labels.length > 0) breadcrumbToRoute.set(labels.join(' → '), route);
  }
  return { reportFile: file, routes, allControls: [...allControls], breadcrumbToRoute };
}

/** 从遍历证据里取某页的控件清单（找不到就返回空数组，绝不编造）。 */
export function controlsOfPage(ev: TraversalEvidence | null, route: string): string[] {
  if (!ev || !route) return [];
