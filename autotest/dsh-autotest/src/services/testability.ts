...
export const CLASS_LABEL: Record<TestabilityClass, string> = {
...
const EXTERNAL_SIGNALS: Array<{ re: RegExp; label: string }> = [
 ...
 * 否则就成了"不想测就判 D"的后门。
 */
export function detectUntestable(c: CaseFacts, sym: SymbolFacts): { isD: boolean; reasons: string[] } {
  const text = [c.name, c.expected, ...c.steps, sym.signature].join('\n');
  const reasons: string[] = [];
  for (const s of EXTERNAL_SIGNALS) if (s.re.test(text)) reasons.push(s.label);
  if (LONG_RUN_RE.test(text)) reasons.push('用例要求超长时运行（≥10 分钟），不适合放进常规回归');
...
export function requiredInputsOf(c: CaseFacts, sym: SymbolFacts): string[] {
 ...
 */
export function referencedControls(steps: string[]): string[] {
  const out: string[] = [];
...
export function classifyCase(c: CaseFacts, sym: SymbolFacts | null, demo: DemoFacts): Verdict {
  ...
  if (used.length > 0) {
    ...
    if (missing.length > 0) evidence.push(`真机上未找到：${missing.join(' / ')}`);
  }
  ...
  if (demo.demoCalls.length === 0) {
    ...
    if (demo.testCallCount > 0) evidence.push(`仅单元测试调用 ${demo.testCallCount} 处（真机路径仍不可达）`);
    return {
      class: 'C',
...
export function scenarioValueFor(scenario: string, current: string): { value: string; note: string } {
  switch (scenario) {
    case 'empty':
 ...
 */
export function extractTemplateLiteral(source: string, lineNo: number): { text: string; startLine: number } | null {
  const lines = source.split(/\r?\n/);
  const startIdx = Math.max(0, lineNo - 1);
  // 从锚点行往下找第一个反引号
  let abs = 0;
  for (let i = 0; i < startIdx; i++) abs += lines[i].length + 1;
  ...
  let line = 1;
  for (let k = 0; k < open; k++) if (source[k] === '\n') line++;
...
export function draftPatch(
  c: CaseFacts, sym: SymbolFacts, demo: DemoFacts, verdict: Verdict,
  readFile: (rel: string) => string | null,
): PatchDraft {
  const base: PatchDraft = {
    class: verdict.class,
  ...
  if (verdict.class === 'C') {
    ...
    if (demo.demoCalls.length === 0 && !callFile) {
      ...
      if (anchorIdx < 1) return { ...base, reason: `${verdict.reason}；未找到安全的插入锚点，补丁需人工编写`, target: page };
      return {
        ...base,
        target: page,
        edits: [{
    ...
    if (!before.trim()) return { ...base, reason: `${verdict.reason}；调用点行为空，补丁需人工编写`, target: callFile };
    return {
      ...base,
      target: callFile,
      edits: [{
...
// ---------- 5. 补丁的校验 / 应用 / 回退（只在独立副本上） ----------

export interface ApplyResult {
  ok: boolean;
  copyDir: string;
  applied: PatchEdit[];
  failed: Array<{ edit: PatchEdit; reason: string }>;
  manifestFile: string;
}

/** 判断目标路径是否在副本目录内（防目录穿越；补丁绝不能写到副本之外）。 */
export function isInside(copyRoot: string, rel: string): boolean {
 ...
 */
export function applyPatchToCopy(copyRoot: string, patch: PatchDraft): ApplyResult {
  const applied: PatchEdit[] = [];
  const failed: Array<{ edit: PatchEdit; reason: string }> = [];
  for (const e of patch.edits) {
    ...
    if (!fs.existsSync(abs)) { failed.push({ edit: e, reason: `文件不存在：${e.file}` }); continue; }
...
export function revertCopy(copyRoot: string): { ok: boolean; removed: string[] } {
  const manifestFile = path.join(copyRoot, 'autotest-patch-manifest.json');
  if (!fs.existsSync(manifestFile)) return { ok: false, removed: [] };
  const removed: string[] = [];
  try {
    const m = JSON.parse(fs.readFileSync(manifestFile, 'utf8')) as { edits?: PatchEdit[] };
    for (const e of m.edits ?? []) {
      if (!isInside(copyRoot, e.file)) continue;
      const abs = path.resolve(copyRoot, e.file);
      if (!fs.existsSync(abs)) continue;
      const content = fs.readFileSync(abs, 'utf8');
      const occurrences = content.split(e.after).length - 1;
      if (occurrences !== 1) continue;      // 已被人工改过，不强行还原
...
export function patchDirFor(libName: string, caseNo: string): string {
...
export function makeCopy(srcDir: string, destDir: string): { ok: boolean; files: number; reason?: string } {
  if (!fs.existsSync(srcDir)) return { ok: false, files: 0, reason: `源目录不存在：${srcDir}` };
  ...
  const walk = (from: string, to: string, depth: number): void => {
    ...
    for (const e of fs.readdirSync(from, { withFileTypes: true })) {
      ...
      if (e.isDirectory()) walk(s, d, depth + 1);
      else if (e.isFile()) { fs.copyFileSync(s, d); files++; }
    }
  };
  try {
    ...
    return { ok: false, files, reason: (e as Error).message };
  }
}
...
export interface TestabilityRunResult {
  ...
  humanQueue: Array<{ caseNo: string; name: string; class: TestabilityClass; reason: string }>;
  withPatch: number;
  details: Array<{ caseId: number; caseNo: string; class: TestabilityClass; reason: string; hasPatch: boolean }>;
}
 ...
 */
export async function runTestability(libraryId: number): Promise<TestabilityRunResult> {
  const db = getDb();
  const lib = await db.prepare('SELECT id, name, repo_url, repo_subpath, last_commit FROM libraries WHERE id = ?')
    .get<{ id: number; name: string; repo_url: string; repo_subpath: string; last_commit: string }>(libraryId);
  ...
  const cases = await db.prepare(`SELECT id, case_no, name, steps, expected, scenario_kind, api_symbol_id FROM cases WHERE library_id = ? ORDER BY id`)
    .all<{ id: number; case_no: string; name: string; steps: string; expected: string; scenario_kind: string; api_symbol_id: number | null }>(libraryId);
  const symbols = await db.prepare('SELECT * FROM api_symbols WHERE library_id = ? ORDER BY id').all<Record<string, unknown>>(libraryId);
  ...
  const assets = await db.prepare(`SELECT kind, name, page_path, source_file, source_line, snippet FROM demo_assets
    WHERE library_id = ? AND library_version = ?`).all<{ kind: string; name: string; page_path: string; source_file: string; source_line: number; snippet: string }>(libraryId, version);
  const matrix = await db.prepare('SELECT symbol_id, evidence_json FROM coverage_matrix WHERE library_id = ?')
    .all<{ symbol_id: number; evidence_json: string }>(libraryId);
  const evidenceBySymbol = new Map(matrix.map((m) => [m.symbol_id, JSON.parse(m.evidence_json || '{}') as { deviceControls?: string[]; paramPoints?: Array<{ pagePath: string; name: string; sourceFile: string; sourceLine: number }> }]));
  ...
  const readFile = (rel: string): string | null => {
    ...
    if (!abs.startsWith(libDir)) return null;      // 防越界读取
    try { return fs.existsSync(abs) ? fs.readFileSync(abs, 'utf8') : null; } catch { return null; }
  };
  ...
  const byClass: Record<TestabilityClass, number> = { A: 0, B: 0, C: 0, D: 0 };
  const humanQueue: TestabilityRunResult['humanQueue'] = [];
  const details: TestabilityRunResult['details'] = [];
  let withPatch = 0;
  const t = now();

  for (const c of cases) {
    const symRow = c.api_symbol_id ? symbolById.get(c.api_symbol_id) : undefined;
    const sym: SymbolFacts | null = symRow ? {
    ...
    const facts: CaseFacts = {
      ...
      steps: (() => { try { return JSON.parse(c.steps || '[]') as string[]; } catch { return []; } })(),
      expected: c.expected ?? '',
    };
    ...
    if (sym && (verdict.class === 'B' || verdict.class === 'C')) {
      ...
      if (draft.edits.length > 0) { patch = JSON.stringify(draft); withPatch++; }
      else patch = JSON.stringify(draft);   // 无 edits 也存下来：里面写着"为什么没有自动补丁"，人要看
    }
...
export async function applyCasePatch(caseId: number): Promise<{
  ok: boolean; copyDir: string; files: number; applied: PatchEdit[]; failed: Array<{ edit: PatchEdit; reason: string }>; reason?: string;
}> {
  const db = getDb();
  const row = await db.prepare(`SELECT c.id, c.case_no, c.demo_patch_json, l.id AS lib_id, l.name AS lib_name, l.repo_url, l.repo_subpath
    ...
    .get<{ id: number; case_no: string; demo_patch_json: string; lib_id: number; lib_name: string; repo_url: string; repo_subpath: string }>(caseId);
  if (!row) throw Object.assign(new Error('用例不存在'), { statusCode: 404 });
  if (!row.demo_patch_json) throw Object.assign(new Error('该用例没有补丁草案（只有 B/C 类才有）'), { statusCode: 400 });
  const patch = JSON.parse(row.demo_patch_json) as PatchDraft;
  if (patch.edits.length === 0) throw Object.assign(new Error(`该用例的补丁草案没有可自动应用的改动：${patch.reason}`), { statusCode: 400 });
  ...
  if (!copied.ok) throw Object.assign(new Error(`复制 demo 失败：${copied.reason}`), { statusCode: 500 });
...
export async function revertCasePatch(caseId: number, removeCopy = false): Promise<{ ok: boolean; removed: string[]; copyDir: string }> {
  const db = getDb();
  const row = await db.prepare('SELECT c.case_no, l.name AS lib_name FROM cases c JOIN libraries l ON l.id = c.library_id WHERE c.id = ?')
    .get<{ case_no: string; lib_name: string }>(caseId);
  if (!row) throw Object.assign(new Error('用例不存在'), { statusCode: 404 });
  const copyDir = patchDirFor(row.lib_name, row.case_no);
  const res = revertCopy(copyDir);
  if (removeCopy) fs.rmSync(copyDir, { recursive: true, force: true });
  return { ...res, copyDir };
}
