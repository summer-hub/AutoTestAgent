...
import { evaluateQuality, validateOracles, type Oracle, type OracleProblem } from '../services/oracle.js';
...
import {
  canConfirm, loadEntriesFromDisk, rebuildIndex, renderInjection, retrieve, saveEntry, wikiRoot, type KnowledgeEntry, type KnowledgeStatus,
} from '../services/knowledge.js';
import {
  AGENT_KIND_LABEL, bindingTraceLine, deleteBinding, externalCmdAvailable, JSONRPC, listResolvedBindings, mcpInitializeResult, mcpTools,
  rpcError, rpcResult, STAGES, upsertBinding, type AgentKind, type JsonRpcRequest,
} from '../services/agentBinding.js';
...
function defineRoutes(llm: LlmCall): void {
  ...
  route('POST', '/libraries/:id/extract-api', async ({ params }) => {
    ...
    const row = await getDb().prepare('SELECT id, name, repo_url, repo_subpath, last_commit, current_version FROM libraries WHERE id = ?')
      .get<{ id: number; name: string; repo_url: string; repo_subpath: string; last_commit: string; current_version: string }>(id);
    if (!row) throw Object.assign(new Error('库不存在'), { statusCode: 404 });
    const r = await runApiExtraction(row);
    if (!r.ok) throw Object.assign(new Error(r.reason ?? '接口提取失败'), { statusCode: 400 });
  ...
  route('GET', '/libraries/:id/api-symbols', async ({ params, query }) => {
    const id = Number(params.id);
    const row = await getDb().prepare('SELECT id, name FROM libraries WHERE id = ?').get<{ id: number; name: string }>(id);
    if (!row) throw Object.assign(new Error('库不存在'), { statusCode: 404 });
    const { version, symbols } = await loadStoredSymbols(id, query.get('version') ?? undefined);
    const demoAssets = await getDb().prepare(
      `SELECT kind, name, page_path, source_file, source_line, snippet, mutability
       FROM demo_assets WHERE library_id = ? AND library_version = ? ORDER BY kind, name, source_line LIMIT 2000`,
    ).all<Record<string, unknown>>(id, version);
  ...
  route('GET', '/knowledge', async ({ query }) => {
    ...
    const filtered = entries.filter((e) =>
      (!scopeKind || e.scopeKind === scopeKind)
      && (!kind || e.kind === kind)
  ...
  route('POST', '/knowledge', async ({ body }) => {
    const b = (body ?? {}) as Partial<KnowledgeEntry>;
    if (!b.title || !b.scopeKind) throw Object.assign(new Error('缺少 title 或 scopeKind'), { statusCode: 400 });
  ...
  route('POST', '/knowledge/confirm', async ({ body }) => {
    ...
    if (!e) throw Object.assign(new Error('知识条目不存在'), { statusCode: 404 });
    // 生命周期纪律：没有证据的结论不允许标为人工确认（知识库不能收传闻）
    const can = canConfirm(e);
    if (!can.ok) throw Object.assign(new Error(can.reason), { statusCode: 400 });
  ...
  route('POST', '/knowledge/status', async ({ body }) => {
    const b = (body ?? {}) as { id?: string; status?: KnowledgeStatus };
    const { entries } = loadEntriesFromDisk();
    const e = entries.find((x) => x.id === b.id);
    if (!e) throw Object.assign(new Error('知识条目不存在'), { statusCode: 404 });
    ...
    return await saveEntry({ ...e, status: b.status as KnowledgeStatus });
  });
  ...
  route('POST', '/knowledge/retrieve', async ({ body }) => {
    const b = (body ?? {}) as { library?: string; apiName?: string; scenarioKind?: string; stage?: string; budgetChars?: number };
    const { entries } = loadEntriesFromDisk();
  ...
  route('POST', '/agent/bindings', async ({ body }) => {
    const b = (body ?? {}) as {
      stage?: string; scope?: 'global' | 'library'; libraryId?: number; kind?: AgentKind;
    ...
    const r = await upsertBinding({
      ...
      kind: (b.kind ?? 'builtin') as AgentKind, promptId: b.promptId ?? null, skillPath: b.skillPath,
  ...
  route('POST', '/mcp', async ({ body }) => {
    ...
    if (req.method === 'initialize') return rpcResult(req.id, mcpInitializeResult());
    if (req.method === 'tools/list') return rpcResult(req.id, { tools: mcpTools() });
  ...
  route('GET', '/libraries/:id/script-bindings', async ({ params }) => {
    const id = Number(params.id);
    const lib = await getDb().prepare('SELECT id FROM libraries WHERE id = ?').get<{ id: number }>(id);
    if (!lib) throw Object.assign(new Error('库不存在'), { statusCode: 404 });
    const bindings = await listBindings(id);
    const byStatus: Record<string, number> = {};
    for (const b of bindings) byStatus[b.status] = (byStatus[b.status] ?? 0) + 1;
  ...
  route('POST', '/cases/:id/script/unbind', async ({ params, body }) => {
    const removeFile = (body as { removeFile?: boolean } | undefined)?.removeFile !== false;
  ...
  route('POST', '/libraries/:id/triage', async ({ params, body }) => {
    const id = Number(params.id);
    const maxDurationSec = Number((body as { maxDurationSec?: number } | undefined)?.maxDurationSec ?? 0) || undefined;
  ...
  route('GET', '/libraries/:id/human-queue', async ({ params, query }) => {
    ...
    for (const i of items) byStage[i.stage] = (byStage[i.stage] ?? 0) + 1;
  ...
  route('POST', '/human-queue/:id/resolve', async ({ params, body }) => {
    const id = Number(params.id);
    const b = (body ?? {}) as { resolution?: string; resolvedBy?: string };
    const resolution = String(b.resolution ?? '').trim();
    if (!resolution) throw Object.assign(new Error('回填结论不能为空（结论会沉淀为知识条目，空结论等于没处理）'), { statusCode: 400 });
  ...
  route('GET', '/cases/:id/patch', async ({ params }) => {
    ...
    const row = await getDb().prepare(`SELECT c.id, c.case_no, c.name, c.testability, c.testability_reason, c.demo_patch_json
      ...
      .get<{ id: number; case_no: string; name: string; testability: string; testability_reason: string; demo_patch_json: string }>(id);
    if (!row) throw Object.assign(new Error('用例不存在'), { statusCode: 404 });
    let patch: unknown = null;
    try { patch = row.demo_patch_json ? JSON.parse(row.demo_patch_json) : null; } catch { patch = null; }
    return {
  ...
  route('POST', '/cases/:id/patch/apply', async ({ params, body }) => {
    const id = Number(params.id);
    const approved = (body as { approved?: boolean } | undefined)?.approved === true;
    if (!approved) {
  ...
  route('POST', '/cases/:id/patch/revert', async ({ params, body }) => {
    const id = Number(params.id);
    const removeCopy = (body as { removeCopy?: boolean } | undefined)?.removeCopy === true;
  ...
  route('POST', '/cases/:id/links', async ({ params, body }) => {
    ...
    const b = (body ?? {}) as { symbolId?: number; linked?: boolean };
    const symbolId = Number(b.symbolId);
    if (!Number.isFinite(symbolId) || symbolId <= 0) throw Object.assign(new Error('symbolId 必填'), { statusCode: 400 });
    const r = await setManualLink(caseId, symbolId, b.linked !== false);
    if (!r.ok) throw Object.assign(new Error(r.message), { statusCode: 404 });
    return { ok: true, message: r.message, links: await listLinks(Number((await getDb().prepare('SELECT library_id FROM cases WHERE id = ?').get<{ library_id: number }>(caseId))?.library_id ?? 0)) };
  });
  ...
  route('POST', '/scripts/run', async ({ body }) => {
    ...
    if (!pythonCmd) throw Object.assign(new Error(describePythonProbe(await probePythons())), { statusCode: 400 });
...
async function callMcpTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const num = (v: unknown, label: string): number => {
    const n = Number(v);
    if (!Number.isFinite(n) || n <= 0) throw Object.assign(new Error(`${label} 必须是正整数`), { statusCode: 400 });
    return n;
  };
  switch (name) {
    ...
    case 'autotest.case_read': {
      ...
      if (args.libraryId !== undefined) {
        ...
        const rows = await getDb().prepare(`SELECT id, case_no, name, steps, expected, status, scenario_kind, priority, testability, oracle_json
          FROM cases WHERE library_id = ? ORDER BY id LIMIT ?`).all<Record<string, unknown>>(id, limit);
        return { libraryId: id, cases: rows.map((r) => mapCase(r)) };
      }
      if (args.libraryName !== undefined) {
        const lib = await getDb().prepare('SELECT id FROM libraries WHERE name = ?').get<{ id: number }>(String(args.libraryName));
        if (!lib) throw Object.assign(new Error(`库不存在：${String(args.libraryName)}`), { statusCode: 404 });
        const rows = await getDb().prepare(`SELECT id, case_no, name, steps, expected, status, scenario_kind, priority, testability, oracle_json
          FROM cases WHERE library_id = ? ORDER BY id LIMIT ?`).all<Record<string, unknown>>(lib.id, limit);
    ...
    case 'autotest.case_write': {
      ...
      if (problems.length > 0) {
        throw Object.assign(new Error(`oracle 不达标，拒绝写入：${problems.map((p: OracleProblem) => p.reason).join('；')}`), { statusCode: 400 });
      }
      const steps = Array.isArray(args.steps) ? (args.steps as unknown[]).map(String) : [];
      if (steps.length === 0) throw Object.assign(new Error('steps 不能为空'), { statusCode: 400 });
      const created = await insertDraftCases(id, [{
        ...
        priority: String(args.priority ?? 'P1'), oracles: args.oracles as Oracle[],
...
function mapCase(row: Record<string, unknown>, libraryName?: string) {
  ...
  return {
    ...
    scenarioKind: (row.scenario_kind as string | undefined) ?? '',
    priority: (row.priority as string | undefined) ?? '',
    apiSymbolId: (row.api_symbol_id as number | null | undefined) ?? null,
    testability: (row.testability as string | undefined) ?? '',
    testabilityReason: (row.testability_reason as string | undefined) ?? '',
    hasPatch: !!String(row.demo_patch_json ?? ''),
    // P6：机器可校验判据（断言覆盖率硬门槛；空数组=该用例断言为空）
    oracles: (() => { try { return JSON.parse(String(row.oracle_json ?? '[]')); } catch { return []; } })(),
