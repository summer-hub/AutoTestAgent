// 种子数据（插件版）：400 三方库 × 每库 ≥100 用例（含版本历史、任务、计划、设备、Prompt、配置、模型）
...
export function seed(db: Database.Database): void {
  ...
  const SOURCES: Array<['新需求引入' | '老库存量' | '问题单跟踪' | 'AI 生成', number]> = [
    ['老库存量', 0.30], ['新需求引入', 0.35], ['问题单跟踪', 0.20], ['AI 生成', 0.15],
  ];
  const CASE_STATUSES: Array<['通过' | '失败' | '待确认' | '未执行', number]> = [
  ...
  function weighted<T extends string>(items: Array<[T, number]>): T {
    const r = Math.random();
    let acc = 0;
    for (const [v, w] of items) { acc += w; if (r <= acc) return v; }
    return items[0][0];
  }
  ...
  db.prepare(`INSERT OR IGNORE INTO settings (key, value, updated_at) VALUES
   ...
   ('device.execEngine', '"reserved"', @t)`).run({ t: t1 });
