...
/** 各分片库数/用例数统计（验证分片均匀性）。 */
export function shardStats(): Array<{ shard: number; libraries: number; cases: number }> {
  const db = getDb();
  const rows = db.prepare('SELECT library_id, COUNT(*) AS n FROM cases GROUP BY library_id').all() as Array<{ library_id: number; n: number }>;
