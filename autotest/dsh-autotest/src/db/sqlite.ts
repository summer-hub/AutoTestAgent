...
function defaultDataDir(): string {
  ...
  for (let i = 0; i < 32; i++) {
    ...
    if (parent === dir) break;
    if (path.basename(parent) === 'node_modules') installRoot = path.dirname(parent);
    dir = parent;
  }
 ...
 */
function adoptLegacyData(target: string): string {
  if (!fs.existsSync(path.join(LEGACY_DATA_DIR, DB_FILE))) return target;
  if (fs.existsSync(path.join(target, DB_FILE))) return target; // 已经搬过
  try {
  ...
  try {
    ...
    if (!fs.existsSync(path.join(target, DB_FILE))) throw new Error('副本里没有库文件');
    try {
    ...
    if (fs.existsSync(path.join(target, DB_FILE))) return target;
