...
export function dbPath(): string {
  if (process.env.AUTOTEST_DB) return process.env.AUTOTEST_DB;
...
let readPool: Database.Database[] | null = null;
let readCursor = 0;
...
export function withRead<T>(fn: (db: Database.Database) => T): T {
  if (!readPool) {
...
export function ensureSchemaAndSeed(): void {
  ...
  const badSettings = db.prepare('SELECT key, value FROM settings').all() as Array<{ key: string; value: string }>;
