...
const sqliteTxStore = new AsyncLocalStorage<true>();
let sqliteTxOpen = false;
let sqliteTxWaiters: Array<() => void> = [];

/** 挂起直到当前事务释放（由调用方在 while 条件里同步检查，避免"检查—置位"被 await 拆开）。 */
function sqliteIdleTick(): Promise<void> {
  return new Promise<void>((resolve) => { sqliteTxWaiters.push(resolve); });
}
 ...
 */
async function acquireSqliteTx(): Promise<void> {
  while (sqliteTxOpen) await sqliteIdleTick();
  sqliteTxOpen = true; // 与上面的条件检查处于同一同步块 → 原子
}

function releaseSqliteTx(): void {
  sqliteTxOpen = false;
...
export async function transaction<T>(fn: () => Promise<T>): Promise<T> {
  if (dbMode() === 'sqlite') {
    ...
    try {
      ...
      if (begun) { try { sqlite().exec('ROLLBACK'); } catch { /* ignore */ } }
      throw e;
    } finally {
      releaseSqliteTx();
    }
 ...
 */
export const DEMO_STAGE_PROMPTS: Array<{ name: string; role: string; content: string; skill: string }> = [
  {
...
async function upgradeBuiltinPrompts(): Promise<void> {
  try {
    await getDb().prepare(
      `UPDATE prompts SET content = ?, skill = ?, variables = '[]', version = 3, updated_at = ?
       WHERE role = '用例生成' AND builtin = 1 AND version < 3`,
    ).run(CASE_GEN_V3_CONTENT, CASE_GEN_V3_SKILL, now());
  } catch (e) {
  ...
  try {
    await getDb().prepare(
      `UPDATE prompts SET content = ?, skill = ?, variables = '[]', version = 2, updated_at = ?
       WHERE role = '用例优化' AND builtin = 1 AND version < 2`,
    ).run(CASE_OPT_V2_CONTENT, CASE_OPT_V2_SKILL, now());
  } catch (e) {
    console.warn('[dsh-autotest] 用例优化 Prompt 升级跳过：', (e as Error).message);
  }
  ...
  try {
    ...
    console.warn('[dsh-autotest] 用例优化 Agent 补插跳过：', (e as Error).message);
  }
  ...
  try {
    ...
    console.warn('[dsh-autotest] 脚本生成 Agent 补插跳过：', (e as Error).message);
  }
  ...
  for (const row of DEMO_STAGE_PROMPTS) {
    try {
      ...
      console.warn(`[dsh-autotest] ${row.role} 绑定补插跳过：`, (e as Error).message);
    }
  }
}
    ...
    readyPromise = (async () => {
      ...
      try {
        await mysqlPool().query(
          `UPDATE prompts SET content = ?, skill = ?, variables = '[]', version = 3, updated_at = NOW()
           WHERE role = '用例生成' AND builtin = 1 AND version < 3`,
          [CASE_GEN_V3_CONTENT, CASE_GEN_V3_SKILL],
        );
      } catch (e) {
        console.warn('[dsh-autotest] 内置 Prompt 升级跳过：', (e as Error).message);
      }
